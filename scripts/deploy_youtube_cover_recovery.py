...
EXPECTED = {
    'features/youtube_auto_publish/service.py': '43541991863b63c22f0c7e84c28dfe6704e6e00cecf160b136d14373eeb5fca9',
    'features/youtube_auto_publish/runtime.py': '196e481a1abde507c49baf84a1bb7407e7fbe52cd8e1164b48e44262a2c517ea',
    'static/youtube-publish.html': 'e37fe3819602d63c2e0b2146f9ffd6c17e49a3d27a42f28b59fbc1bff8e0c0a0',
    'static/youtube-publish.js': 'f8146f0ad0d6af826a73af19dcf307f6cee087e99d65bd4d00201d99f1f9be63',
}
...
def install(source, target, mode):
    target = Path(target)
    temp = target.with_name(target.name + '.youtube-cover-recovery-new')
...
def main():
    ...
    with (BASE / 'cover-recovery-deploy.lock').open('a') as lock:
        ...
        if args.rollback:
            ...
            if manifest.get('kind') != 'youtube-cover-recovery' or {r['target'] for r in manifest['files']} != {str(t) for t in targets}:
        ...
        backup = BASE / 'backups' / ('cover-recovery-' + time.strftime('%Y%m%d-%H%M%S') + '-' + args.commit[:12])
        ...
        manifest = {'kind': 'youtube-cover-recovery', 'commit': args.commit, 'files': records, 'ledger_before': before}
