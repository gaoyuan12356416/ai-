#!/usr/bin/env python3
"""Install two additive cover modules; drain only the automatic YouTube worker."""
import argparse
import fcntl
import hashlib
import json
...
# Dependency first: the old runtime also works with the new images module.
EXPECTED = {
    'features/youtube_auto_publish/images.py': '2cf965bf2077ea02af600e0a921b0c1867bc3fcbdbd689e89975dfeed74999fd',
    'features/youtube_auto_publish/runtime.py': '77d456a408498737b6af9a8d2dd814caad279a5ff26ccaac2eaa34e7242e8717',
}
...
def install(source, target, mode):
    temp = target.with_name(target.name + '.crop-new')
...
def main():
    ...
    with (BASE/'cover-crop-deploy.lock').open('a') as lock:
        ...
        if args.rollback:
            ...
            rows = manifest['files']
            if manifest.get('kind') != 'youtube-cover-crop' or {r['path'] for r in rows} != set(EXPECTED):
            ...
            # Restore old runtime before removing its new dependency.
        ...
        backup = BASE/'backups'/('cover-crop-' + time.strftime('%Y%m%d-%H%M%S') + '-' + args.commit[:12])
        ...
        manifest = {'kind': 'youtube-cover-crop', 'commit': args.commit, 'files': rows, 'ledger_before': before}
