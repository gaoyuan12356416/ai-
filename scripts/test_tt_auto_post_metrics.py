#!/usr/bin/env python3
"""Offline tests for TT auto-post metric refresh and cache reads."""
...
from scripts.tt_auto_post_metric_runner import (  # noqa: E402
    MetricRunnerConfig,
...
class MetricRefreshTests(unittest.TestCase):
    ...
    def test_real_store_refresh_and_window_reader_contract(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            store = TTPostAutoStore(str(Path(directory) / "auto.sqlite3"))
...
class MetricRunnerTests(unittest.TestCase):
    ...
    def test_config_defaults_to_dramawave_platform_zero_and_redacts_secret(self):
        config = MetricRunnerConfig.from_env(
            {
                "TT_AUTO_POST_MYSQL_USER": "readonly",
                "TT_AUTO_POST_MYSQL_PASSWORD": "top-secret",
            }
        )
            ...
            MetricRunnerConfig.from_env(
                {
                    "TT_AUTO_POST_MYSQL_USER": "readonly",
                    "TT_AUTO_POST_MYSQL_PASSWORD": "top-secret",
                    "TT_AUTO_POST_PRODUCT": "OtherProduct",
                }
            )
        with self.assertRaisesRegex(ValueError, "configuration is invalid"):
            MetricRunnerConfig.from_env(
                {
                    "TT_AUTO_POST_MYSQL_USER": "readonly",
                    "TT_AUTO_POST_MYSQL_PASSWORD": "top-secret",
                    "TT_AUTO_POST_METRIC_PLATFORM": "1",
                }
            )
    ...
    def test_config_reuses_existing_tt_post_read_only_mysql_credentials(self):
        config = MetricRunnerConfig.from_env(
            {
                "TT_POST_MYSQL_HOST": "101.32.56.53",
                "TT_POST_MYSQL_PORT": "63350",
                "TT_POST_MYSQL_USER": "existing-readonly",
                "TT_POST_MYSQL_PASSWORD": "existing-secret",
                "TT_POST_MATERIAL_MYSQL_DATABASE": "kunlunads_dev",
            }
        )
        self.assertEqual(config.mysql_user, "existing-readonly")
        self.assertEqual(config.mysql_password, "existing-secret")
        self.assertEqual(config.mysql_database, "kunlunads_dev")
        self.assertNotIn("existing-secret", repr(config))
        with self.assertRaisesRegex(ValueError, "configuration is invalid"):
            MetricRunnerConfig.from_env(
                {
                    "TT_POST_MYSQL_HOST": "attacker.invalid",
                    "TT_POST_MYSQL_PORT": "63350",
                    "TT_POST_MYSQL_USER": "existing-readonly",
                    "TT_POST_MYSQL_PASSWORD": "existing-secret",
                    "TT_POST_MATERIAL_MYSQL_DATABASE": "kunlunads_dev",
