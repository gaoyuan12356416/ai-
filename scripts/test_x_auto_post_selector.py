#!/usr/bin/env python3
"""Offline tests for deterministic X auto-post two-stage selection."""
...
from features.x_auto_posts.selector import (  # noqa: E402
    CandidateRejected,
    NoEligibleMaterial,
    SelectionError,
    SelectionRequest,
...
class FakeStore:
    ...
    def reserve_material(self, **kwargs):
        ...
        if content_id in self.cooldown_conflict_once:
            ...
            raise StoreConflict("x_auto_drama_in_cooldown")
        if material_id in self.conflict_once:
            self.conflict_once.remove(material_id)
            raise StoreConflict("x_auto_material_already_reserved")
...
def selector(source, metric_store, history=None, store=None, validator=None):
    return TwoStageSelector(
        source,
        MetricWindowRepository(metric_store),
        history or FakePublishedHistory(),
        store or FakeStore(),
...
class TwoStageSelectionTests(unittest.TestCase):
    ...
    def test_preview_validates_and_ranks_without_reservation(self):
        source = FakeSource(
            [drama("C1")],
            {"C1": [material("101", "C1")]},
        )
        ...
        self.assertEqual(selected.material.source.material_id, "101")
        self.assertEqual(selected.reservation, {})
        self.assertEqual(store.reserve_calls, [])
        self.assertEqual(store.reserved, set())

    def test_published_history_auto_and_reservation_race_all_filter_before_return(self):
        source = FakeSource(
            [drama("C1")],
            {
                "C1": [
        ...
        history = FakePublishedHistory({"101"})
        store = FakeStore()
    ...
    def test_strict_public_metadata_is_frozen_and_sensitive_fields_are_omitted(self):
        ...
        self.assertNotIn("source_media_url", frozen)
        self.assertNotIn("media_url", frozen)
