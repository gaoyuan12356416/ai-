...
EXPECTED = {
    'app.py': '7a8ba1a5cbe0ce3dbfdad108444743bb220e09c0525945b39924a1aa977dc743',
    'static/youtube-publish.html': '149a4253d4164a91d7085ffe057a61205849926da9f23044ebd858e9904f3d7e',
    'static/youtube-short-links.js': None,
    'static/youtube-short-links.css': None,
    'features/youtube_auto_publish/manual_links.py': None,
    'features/youtube_auto_publish/manual_link_routes.py': None,
}
...
def patch_app(current):
    anchor = b'        if "/x-share" in path:\n'
    addition = (b'        if path == "/short-links" or path.startswith("/short-links/"):\n'
                b'            from features.youtube_auto_publish.manual_link_routes import dispatch\n'
                b'            return dispatch(self, parsed, actor, globals())\n')
    if current.count(anchor) != 1 or addition in current:
        raise RuntimeError('Application route anchor mismatch')
    return current.replace(anchor, addition + anchor, 1)
...
def install(path, data, mode):
    ...
    temp = path.with_name(path.name + '.manual-link-new')
...
def main():
    ...
    if args.check:
        ...
        return
    backup = BASE/'backups'/('manual-links-'+time.strftime('%Y%m%d-%H%M%S')+'-'+commit[:12])
