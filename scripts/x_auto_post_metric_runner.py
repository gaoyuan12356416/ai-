#!/usr/bin/env python3
"""Refresh X automatic-post metric cache one complete Beijing day at a time."""
...
UTC = timezone.utc
DEFAULT_DB_PATH = "/mnt/data-disk/x-auto-post-publisher/x-auto-post.sqlite3"
DEFAULT_MYSQL_HOST = "101.32.56.53"
DEFAULT_MYSQL_PORT = 63350
DEFAULT_LOCK_PATH = "/run/x-auto-post/metric.lock"
        ...
        try:
            port = int(
                source.get(
                    ...
                    source.get("X_POST_SCHEDULE_MYSQL_PORT", DEFAULT_MYSQL_PORT),
                )
            )
            platform = int(source.get("X_AUTO_POST_METRIC_PLATFORM", DEFAULT_PLATFORM))
            lookback = int(source.get("X_AUTO_POST_METRIC_LOOKBACK_DAYS", "7"))
        except (TypeError, ValueError, OverflowError):
            raise ValueError("metric runner numeric configuration is invalid") from None
        config = cls(
            db_path=str(source.get("X_AUTO_POST_DB_PATH", DEFAULT_DB_PATH)).strip(),
            mysql_host=str(
                source.get(
                    "X_AUTO_POST_MYSQL_HOST",
                    source.get("X_POST_SCHEDULE_MYSQL_HOST", DEFAULT_MYSQL_HOST),
                )
            ).strip(),
            mysql_port=port,
            mysql_user=str(
                source.get(
                    ...
                    source.get("X_POST_SCHEDULE_MYSQL_USER", ""),
                )
            ).strip(),
            mysql_password=str(
                source.get(
                    "X_AUTO_POST_MYSQL_PASSWORD",
                    source.get("X_POST_SCHEDULE_MYSQL_PASSWORD", ""),
                )
            ),
            mysql_database=str(
                source.get(
                    "X_AUTO_POST_MYSQL_DATABASE",
                    source.get("X_POST_SCHEDULE_MYSQL_DATABASE", DEFAULT_SCHEMA),
                )
            ).strip(),
            blacklist_database=str(
                source.get(
                    "X_AUTO_POST_BLACKLIST_DATABASE",
                    DEFAULT_BLACKLIST_SCHEMA,
                )
            ).strip(),
            product=str(source.get("X_AUTO_POST_PRODUCT", DEFAULT_PRODUCT)).strip(),
            platform=platform,
            lookback_days=lookback,
            lock_path=str(
                source.get("X_AUTO_POST_METRIC_LOCK_PATH", DEFAULT_LOCK_PATH)
            ).strip(),
        )
        config.validate()
        return config
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh X auto-post daily metric generations",
    )
    parser.add_argument(
        "--date",
        action="append",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        from features.x_auto_posts.core import XAutoPostStore

        store = XAutoPostStore(config.db_path)
        ...
        code = str(getattr(exc, "code", "x_auto_metric_refresh_failed"))
        print(
            json.dumps(
                {
                    "ok": False,
