...
Exact pool/manual videos with a null, empty or zero `video_duration` now download
the source through the existing HTTPS host/type/size gates and read its actual
duration with ffprobe. Existing positive durations and images avoid this extra
...
Temporary files use the existing verified data-disk `media-work` directory and
