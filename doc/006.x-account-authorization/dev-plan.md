...
| 生产真实 Cookie/跨 owner OAuth/revoke 验收 | 用户 + Codex | `ai.yingliangads.com` | Cookie、跨 owner OAuth 与真实 revoke 待执行 |
...
- 当前不标记全量生产验收：真实 Feishu Cookie 浏览器、真实跨 owner OAuth callback 与真实 logout/revoke 未执行。
