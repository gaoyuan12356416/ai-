...
| TC-014 | 敏感数据泄漏扫描 | 完成授权/校验/logout | 搜索 API/HTML/log/audit | 无 Secret/Token/code/state/verifier/Basic header | P0 | 本地扫描通过；部署窗口 journal 敏感字段 0、Nginx callback 查询串 0，真实 callback/revoke 流量仍待验收 |
...
| TC-016 | 服务重启恢复 | 多 owner + disconnected 数据 | 重启两个服务 | owner/快照/状态不丢；disconnected 凭证残留被清理 | P1 | Sidecar 28/28；生产 active 账号/Token 启动保全通过，disconnected 残留分支未实测 |
...
| TC-024 | 跨 owner logout IDOR | A 知道 B 的记录 ID | A POST `/{id}/logout` | 404；不调用 revoke，不改变 B 数据 | P0 | BUG-004/Sidecar 通过；生产模块级其他 owner 返回 404 且 Token hash 不变 |
...
| TC-046 | Admin no-store 与时间展示 | admin Cookie + 有刷新/同步/退出时间的数据 | 请求 API/Nginx 并打开页面 | admin API/Nginx `Cache-Control: no-store`，fetch `cache:no-store`；展示到期/刷新、校验/资料同步、更新/退出时间 | P1 | App/静态/Playwright 及生产未登录 owner/admin no-store 通过；真实 Cookie 页面待验收 |
...
- Sidecar 28/28、App contract 5/5、回填脚本 4/4、Python/JS/JSON、静态检查与 Playwright 三路径均已通过；生产回填/Token 保全、服务/API smoke、模块级查询与真实资料同步通过，真实 Cookie/跨 owner OAuth/revoke/rollback 另行验收。
...
V2 生产迁移与模块级 owner 隔离已有现场证据，但仍不能代替真实 Cookie、真实跨 owner OAuth 或真实 revoke 验收。V1 历史证据继续保留，仅作回滚基线。
