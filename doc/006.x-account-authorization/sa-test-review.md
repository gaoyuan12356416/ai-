...
V2 用例设计和本地验收通过：Sidecar 28/28、App 5/5、backfill 4/4、全量编译/静态及 Playwright 三路径通过，console 0 error。生产迁移、部署和真实 X revoke 仍待执行。
...
| STR-003 | 用户文案 | “登录时间/logout”含义可能误导 | 展示授权/同步/退出授权时间；明确不退出 x.com | 已补充，待 UI 验证 |
...
| STR-008 | Owner 隔离 | 只测不同用户，可能漏掉相同 user ID 的跨 tenant 串权 | A/B/C 数据集 + TC-021/022 | 实现已复核；最新套件/生产 Cookie 待验收 |
...
| STR-013 | 资料快照/链接 | 只测完整响应会导致可选字段空值崩溃；链接缺少明确上限 | TC-030/031/032 覆盖完整、缺失及 username 1–50/超长/非法字符 | 最新套件与真实页面待验收 |
| STR-014 | Legacy 迁移 | 只测 schema 或唯一匹配会掩盖跨租户兜底与错误 apply 风险 | TC-042 至 TC-045 覆盖默认 dry-run、唯一/零/多匹配、`--require-all-resolved`、幂等与并发回滚 | 最新脚本套件与生产主库回填待执行 |
...
| STR-017 | 缓存与时间 | admin 数据可能被浏览器/代理缓存，页面可能遗漏刷新/同步/退出时间 | TC-046 同时验证主 API、Nginx、fetch no-store 和两页时间列 | App 5/5、Playwright 通过 |

## QA 修订确认

- V1 用例和历史证据保留。
- V2 用例扩展至 TC-047，并将 BUG-004、BUG-005、pending 退出状态机、启动清理、回填 CLI、缓存/时间与链接边界明确映射到回归。
- 本地 Sidecar 28/28、App 5/5、backfill 4/4、编译/静态和 Playwright 已写入测试报告；生产迁移、部署和真实 revoke 待执行。
