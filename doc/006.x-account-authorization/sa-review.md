...
V1 架构继续采用独立 X OAuth sidecar，凭证只由 sidecar 持有。V2 本地实现与验收通过：页面拆分、联合 owner 隔离、`revoke_pending` 可恢复退出状态机，以及 owner pending 门禁/owner→account 锁序均已落地。Sidecar 28/28、App contract 5/5、backfill 4/4、全量编译/静态检查及 Playwright 三路径（console 0 error）通过；仅生产迁移、部署与真实 X revoke 待执行。
...
| SA-004 | 中 | 权限 | 普通 API Token 可能继承模块权限 | X API 统一要求 Feishu Cookie；admin API 额外要求 admin | App contract 5/5、Playwright 通过；生产待回归 |
...
| SA-011 | P0 | 个人列表/校验 | V1 返回全局列表且 verify 只按记录 ID，可形成跨用户 IDOR | actor 传入 sidecar；SQL 同时过滤 owner tenant/user；admin 使用独立 API | BUG-004 本地已修复/通过；生产待验收 |
| SA-012 | P0 | OAuth callback upsert | V1 `ON CONFLICT(x_user_id)` 会覆盖其他后台用户的 Token 和授权人 | `x_user_id` 全局唯一但归属不可转移；跨 owner 返回 `x_account_owned_by_other`，原数据不变 | BUG-005 本地已修复/通过；生产待验收 |
...
| SA-016 | 高 | Legacy 迁移 | 添加 owner 列可能使生产记录不可见或丢 Token | 标准脚本默认 dry-run；仅唯一匹配回填；`--require-all-resolved` 阻断；apply 前后断言 Token hash | Backfill 4/4 通过；生产回填待执行 |
| SA-017 | 中 | 账号资料 | 列表逐账号请求 X 会放大成本，超长 username 可能生成不安全链接 | callback/verify 保存快照；主页链接只接受 `[A-Za-z0-9_]{1,50}` | Sidecar 28/28、Playwright 通过 |
| SA-018 | 高 | 启动恢复 | disconnected 行旁残留 live Token/tombstone 可能跨重启保留 | 启动按账号锁删除 live Token 与旧 tombstone | Sidecar 28/28 通过；生产启动待验收 |
...
SA-011 至 SA-019 已写入 V2 requirements、API、测试与部署文档。本地 Sidecar 28/28、App 5/5、backfill 4/4、全量编译/静态及 Playwright 三路径已通过；生产迁移、部署和真实 X revoke 尚未完成。
