...
- 线上共享 monolith 存在并行发布；本次已使用精确 source `3bcf083` 并验证 `app.py` 零差异，后续仍禁止从旧提交整份覆盖。
...
- 2026-07-17：门禁发现线上并行提交 `3bcf083`，无冲突合并数值型账户时区调度修复，最终精确回归 181/181 通过。
- 2026-07-17：从 GitHub target `9e6c5c8` 精确 overlay 至生产；服务器 181/181、服务/timer、自然 tick、生产浏览器 UTC+8 页面和回滚备份均验证通过。
