...
| --- | --- | --- | --- | --- | --- | --- |
| TC-001 | additive migration | V3 生产结构副本 | 连续执行两次 `ensure_storage()` | 高优/manual 字段表索引存在，历史行/queue/log 不变 | P0 | 待生产副本演练 |
...
| TC-036 | 无真实发布部署验收 | 生产备份已完成 | 迁移/服务/HTTP/DB检查 | queue/log/post 数量不增加，timer 和服务正常 | P0 | 待生产部署 |
