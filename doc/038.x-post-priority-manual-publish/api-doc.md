...
- 人工明确选择的源视频若超过 600 秒，只能路由给当前 token 确认为 Premium 的目标账号，并以原片进入完整媒体预检；Premium 时长门禁为 4 小时，v2 `amplify_video` API 文件门禁仍为 512 MiB。生产已用 763.938 秒原片完成一次发布与 API 回读。自动素材池的来源上限不受此接口影响。
