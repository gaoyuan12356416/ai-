...
14. 人工明确选择、源时长大于 600 秒的有效视频可进入既有媒体预检，但只能路由给当前 token 确认为 Premium 的账号；符合媒体合同的原片不得仅因超过 600 秒而进入 GPU 修复。Premium 时长门禁按 4 小时执行，v2 API 文件上限仍为 512 MiB；自动素材池的 `<=600` 秒来源过滤保持不变。
...
- 人工素材的源库时长超过 600 秒时不得伪装成“素材不存在”；若没有 Premium 账号，或原片/必要修复后的媒体不满足 4 小时/512 MiB 及既有编解码门禁，整批仍在建队列前失败关闭。
