...
X 素材池已在发布计划创建前执行真实媒体预检。部分 Dramawave 源视频会因 HEVC、尺寸不合规，或时长超过 X API 的 140 秒上限而失败。这些问题属于可确定修复的媒体规格问题，不应与违规、素材不存在或元数据异常混为一类。

## 目标

- 当 X 预检返回 `invalid_media_codec`、`invalid_media_dimensions` 或 `invalid_media_duration` 时，把原视频提交给 GPU 专用修复 worker。
- GPU 不覆盖原文件；画面保持完整并只在源时长超过 140 秒时保留开头、裁掉 139 秒之后的尾部，生成 H264/yuv420p、逐行、CFR 30fps、AAC-LC 的 X 合格 MP4。
...
- 不修复违规、素材不存在、URL/下载、过短、不可探测时长、帧率、扫描方式或其他错误。
...
1. 自动修复只允许三个触发码：`invalid_media_codec`、`invalid_media_dimensions`、`invalid_media_duration`；worker 重新探测后仅对有限且大于 140 秒的时长执行裁尾，少于 0.5 秒仍失败关闭。
...
6. 输出固定为新的 MP4 对象：竖版 720×1280、横版 1280×720、近方形 720×720；保持比例并补黑边，不做空间裁剪、不拉伸、不生成新画面。唯一允许的内容缩短是把超长视频固定裁到 139 秒。
...
| `media_repair_trigger_code` | 三个允许触发码之一 |
...
3. 原视频 probe 成功则不调用 GPU；仅三个允许错误码进入修复。worker 二次 probe 区分超长与过短，并处理 codec/dimensions 首错掩盖的额外超长。
...
- 三个允许错误码可修复；过短、非有限时长及其他错误保持失败关闭。
- 超长输出时长为 139 秒附近且不超过 140 秒；非超长转码继续保持源时长。
- 已是 `validation_failed` 的未绑定短剧只能通过共享调度锁的精确恢复命令，在 GPU 修复和 CPU 重验成功后原地恢复 `pending`；不得删除重加或重放旧失败批次。
