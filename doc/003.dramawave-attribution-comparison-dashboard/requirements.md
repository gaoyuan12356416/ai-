# 003.Dramawave D7/D10 归因对比看板需求与技术设计

## 背景

Dramawave 原使用 D7 归因窗口；当前目标新口径改为 D10。线上真实表为：

- 旧口径：`kunlunads_dev.ads_app_revenues`
- 新口径：`kunlunads_dev.ads_app_revenues_10d`
- 当前投放统计与维度：`kunlunads_dev.ads_custom_source_insight`

已只读核验 D10 表 schema 与索引和 D30 表一致，可以复用既有映射与缓存结构；当前 D10 最早数据日期是 `2026-08-01`。用户已于 2026-08-07 明确批准把看板起点调整为 `2026-08-01`，不再要求 D10 覆盖 7 月 29～31 日；该边界和独立 D10 缓存现已完成生产切换。

这里的“D7 / D10”是归因窗口。两张 revenue 表内部都同时包含 D0 收入和 D7 累计收入，页面不得把“D7 归因窗口”和“D7 累计收入”混为一个概念。

## 目标

1. 提供从 2026-08-01（北京时间，含当天）开始的 Dramawave 对比看板；后端最小日期、前端兜底日期、边界测试和部署参数必须保持一致，更早日期必须拒绝。
2. 同时展示旧 D7 归因窗口和新 D10 归因窗口下的 D0、D7 累计收入、ROAS 和差异。
3. 支持按时间、Campaign、Ad Set、优化师、国家组、渠道、投放产品、账户等维度筛选、组合分组和导出。
4. 页面查询只读本地 SQLite 缓存，不在用户请求内回源 MySQL。
5. 每 30 分钟刷新今天和昨天，保留近 60 天；另轮转回补一个历史日期，避免 D10 延迟归因使两天前数据永久变旧。
...
1. 用户选择相同日期与维度后，可以并排看到旧 D7 归因和新 D10 归因的收入、ROAS、差额和提升率。
...
3. 默认指标基准为 D0：
   ...
   - `新口径 D0 收入 = ads_app_revenues_10d.revenue_iaa_d0 + revenue_iap_d0`
4. 切换到 D7 累计指标时，分别使用两表的 `revenue_iaa_d7 + revenue_iap_d7`。
5. `收入差额 = D10 - D7`；`提升率 = (D10-D7)/D7`。D7=0 且 D10>0 显示“新增”，两者均为 0 显示“—”。
...
SQLite 使用 WAL、`busy_timeout`，保留可查询最细事实，并同步维护去除 Campaign/Ad Set 的日级筛选汇总和 Campaign 日级汇总。API 按查询维度选择最小可用汇总层，只有 Ad Set 明细/搜索才回退最细事实，避免首屏重复扫描大事实表。刷新器使用流式 MySQL 游标；D7/D10 合并键和待发布事实分别写入本地 `refresh_revenue_stage`、`refresh_fact_stage`，不在内存中保留整日 revenue union 或最终事实副本。三张源表的读取仍位于同一日期级一致性快照，映射与守恒检查在快照结束后执行。全部目标日期先 staging，随后在同一个事务中整体覆盖事实、重建汇总并推进 `data_version`；任一日期失败均保留完整上一版。D10 必须使用全新 SQLite 构建并带 `comparison_window=D10`、`new_attribution_source=kunlunads_dev.ads_app_revenues_10d` 语义标记，不能直接复用或误开历史 D30 数据库；Web/refresh 遇到结构或标记不符必须失败关闭。缓存和日志默认放在 `/mnt/data-disk/dramawave-attribution-comparison/`。
...
- 每轮必须刷新北京时间今天和昨天。
- 每轮再按 cursor 轮转刷新一个更早的缓存日期，使延迟 D10 归因在约 29 小时内被再次吸收。
- 删除早于 `max(2026-08-01, 今天-59天)` 的事实和刷新日志。
- D10 首次上线从 `2026-08-01` 显式 bootstrap 至北京今天；不得在页面请求中隐式补数。
...
1. `2026-08-01` 可查，`2026-07-31` 及更早日期被后端和前端一致拒绝；`/api/meta.minimum_date` 必须为 `2026-08-01`。
2. 重新选择并核验 D10 可用日期内的真实点样本；不得沿用 D30 的 2026-07-29 样本值冒充 D10 验收证据。
...
8. 缓存仅保留近 60 天，并始终不早于 `2026-08-01`。
9. 每个新数据版本提交后先预热默认 7 天/全范围、D0/D7、Campaign/Ad Set/常用筛选和排行；用户可见常用聚合 API 目标 p95 <= 300 ms，未预热低基数组合目标 p95 <= 1 秒，首屏目标 <= 2 秒。真实历史不足 30/60 天时以全可用范围实测，不把缺失历史视为性能证据。
...
- 原需求中的“29号”曾锁定为 `2026-07-29 00:00:00 Asia/Shanghai`；本次经用户批准后，新合同起点改为 `2026-08-01 00:00:00 Asia/Shanghai`。
...
- 仅刷新近 2 天不足以吸收 D10 延迟回流，因此在不改变 30 分钟近 2 天主要求的前提下增加单日轮转回补。
- D10 schema/index 已核验与 D30 相同，但日期覆盖不同；schema 兼容不代表数据边界验收通过。
- 生产现运行 D7/D10 合同；历史 D30 release 和 SQLite 仅作为回滚点，不再承载当前页面。
- 生产代码为 `ec6fba1b7e155be904c4c54791fd491c76e09a9a`，活动缓存最小日期为 `2026-08-01`，refresh timer 固定在 `:04/:34`；自动化、真实点样本、独立缓存、浏览器和自然 timer 均已验收。

## 变更记录

- 2026-08-06：创建需求；完成只读表结构、索引、日期覆盖和 7 月 29 日点样本核验。
- 2026-08-06：SA 决定采用服务端 SQLite 聚合 API，放弃 TT 看板的全量浏览器透视架构。
- 2026-08-07：新口径从 D30 调整为 D10，字段契约改为 `d10_*`，源表改为 `ads_app_revenues_10d`。已核验 D10 schema/index 与 D30 相同，当前最早日期为 2026-08-01；2026-08-06 的 D30 发布与性能数据仅保留为历史基线。
- 2026-08-07：用户批准看板起点改为 `2026-08-01`；后端 `MIN_DATE`、前端 `FALLBACK_MIN_DATE`、测试和部署合同同步调整并完成生产验收。
- 2026-08-07：D7/D10 看板完成生产切换，生产 commit 为 `ec6fba1b7e155be904c4c54791fd491c76e09a9a`；活动 D10 SQLite 独立于历史 D30 回滚库，timer 调整为每小时 `:04/:34`。
