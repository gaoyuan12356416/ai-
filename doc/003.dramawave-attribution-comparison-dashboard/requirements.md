...
已只读核验 D10 表 schema 与索引和 D30 表一致，可以复用既有映射与缓存结构；但当前 D10 最早数据日期是 `2026-08-01`。原需求边界仍为 `2026-07-29`，因此 D10 候选缓存不能覆盖完整请求区间，生产切换被阻塞。待用户决定由源侧补齐 7 月 29～31 日，或接受把看板起点调整为 8 月 1 日。

这里的“D7 / D10”是归因窗口。两张 revenue 表内部都同时包含 D0 收入和 D7 累计收入，页面不得把“D7 归因窗口”和“D7 累计收入”混为一个概念。

## 目标

1. 原业务目标是提供从 2026-07-29（北京时间，含当天）开始的 Dramawave 对比看板；在 D10 源数据补齐前不切换生产。若用户批准起点改为 2026-08-01，必须先同步修改后端最小日期、前端兜底日期、边界测试和文档并通过新一轮评审，不能只改 bootstrap 参数。
...
- 删除早于 `max(经批准的看板起点, 今天-59天)` 的事实和刷新日志。经批准的看板起点只能是源侧补齐后的 `2026-07-29`，或用户明确接受后的 `2026-08-01`。
- D10 首次上线仅在边界决策完成后，从经批准的看板起点显式 bootstrap 至今天；不得在页面请求中隐式补数。
...
1. 若源侧补齐，则当前边界代码下 `2026-07-29` 可查且 `2026-07-28` 被拒绝；若用户批准改起点，则必须先在新提交中把后端/前端边界及测试统一改到 `2026-08-01`，之后验证 `2026-08-01` 可查且更早日期被拒绝。在决策和对应代码提交完成前此项为阻塞而非通过。
...
8. 缓存仅保留近 60 天，并始终不早于经批准的看板起点；边界决策前不得把任一候选起点写成已验收值。
...
- 原需求中的“29号”已锁定为 `2026-07-29 00:00:00 Asia/Shanghai`；它仍是业务期望边界，不代表当前 D10 源已覆盖该日。
...
- 生产继续运行历史 D30 基线，D10 状态为待切换；不得把历史 D30 发布证据表述为 D10 已上线。
- D10 本地自动化 `62/62` 已通过；该结果不覆盖真实点样本、候选 SQLite bootstrap、生产数据、浏览器、原子切换或自然 timer，生产结论仍为 NO-GO。

## 变更记录

- 2026-08-06：创建需求；完成只读表结构、索引、日期覆盖和 7 月 29 日点样本核验。
- 2026-08-06：SA 决定采用服务端 SQLite 聚合 API，放弃 TT 看板的全量浏览器透视架构。
- 2026-08-07：新口径从 D30 调整为 D10，字段契约改为 `d10_*`，源表改为 `ads_app_revenues_10d`。已核验 D10 schema/index 与 D30 相同，但当前最早日期为 2026-08-01；原 2026-07-29 边界阻塞生产切换，待用户决定源侧补齐或起点改为 8 月 1 日。2026-08-06 的 D30 发布与性能数据仅保留为历史基线。
- 2026-08-07：D10 本地自动化 `62/62` 通过；候选库和生产验收未执行，不改变日期边界 NO-GO。
