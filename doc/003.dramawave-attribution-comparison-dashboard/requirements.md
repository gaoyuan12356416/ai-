...
- D10 边界调整后的本地自动化 `63/63` 已通过；该结果不覆盖真实点样本、候选 SQLite bootstrap、生产数据、浏览器、原子切换或自然 timer，生产仍待发布验收。
...
- 2026-08-07：用户批准看板起点改为 `2026-08-01`；后端 `MIN_DATE`、前端 `FALLBACK_MIN_DATE`、测试和部署合同已同步调整。本地自动化 `63/63` 通过，候选库和生产验收仍待执行。
