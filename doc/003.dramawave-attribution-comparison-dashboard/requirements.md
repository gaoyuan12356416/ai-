...
已只读核验 D10 表 schema 与索引和 D30 表一致，可以复用既有映射与缓存结构；当前 D10 最早数据日期是 `2026-08-01`。用户已于 2026-08-07 明确批准把看板起点调整为 `2026-08-01`，不再要求 D10 覆盖 7 月 29～31 日；生产切换仍需经过全新候选缓存、真实数据对账和原子发布验收。
...
- D10 边界与 60 天裁剪回归后的本地自动化 `64/64` 已通过；该结果不覆盖真实点样本、候选 SQLite bootstrap、生产数据、浏览器、原子切换或自然 timer，生产仍待发布验收。
...
- 2026-08-07：用户批准看板起点改为 `2026-08-01`；后端 `MIN_DATE`、前端 `FALLBACK_MIN_DATE`、测试和部署合同已同步调整。本地自动化 `64/64` 通过，候选库和生产验收仍待执行。
