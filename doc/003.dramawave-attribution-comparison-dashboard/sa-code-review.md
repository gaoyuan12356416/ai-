...
有条件通过。数据正确性、只读边界、原子刷新和 API 白名单未发现未解决 P0；发布前仍必须用真实 bootstrap 缓存通过冷/热查询门禁，并在目标 Linux 完成 systemd/Nginx 验证与自然 timer 证据。
...
| CR-005 | P1 | 查询性能 | 30 万行合成事实原实现冷查询约 6.8 秒；生产单日最细事实实测 75,279～130,976 行，单纯 LRU 不可放行 | 事务内维护筛选级/Campaign级日汇总，按查询维度选最小层；bootstrap 后统计优化，真实 30/60 天测 p95 | 代码已调整，生产门禁待执行 |
| CR-006 | P1 | 部署 | unit 未绑定数据盘、Nginx 片段可能放错 context、PyMySQL 未固定环境 | `RequiresMountsFor`、明确 `/etc/nginx/default.d`、独立 pinned venv | 已修复，目标机待验证 |
...
- Windows Python：`43/43` 自动化测试通过，包含汇总守恒、路由回退、排行 singleflight、缓存预热和前端并发契约。
...
- 尚待：真实 bootstrap 后 API p95、`systemd-analyze verify`、`nginx -t`、飞书会话、自然 timer。
