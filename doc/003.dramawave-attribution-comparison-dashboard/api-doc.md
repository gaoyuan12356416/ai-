...
`defaults.start_date` 是缓存范围内最近 7 天的起点。它只是页面建议值；其他接口在未传日期时仍默认使用整个缓存可用范围。D10 合同中的 `minimum_date` / `expected_start_date` 固定为 `2026-08-01`。
...
- 2026-08-07：目标新口径由 D30 改为 D10；所有 API、排序、质量和导出字段从 `d30_*` 改为 `d10_*`，新源表改为 `kunlunads_dev.ads_app_revenues_10d`。用户批准起点为 `2026-08-01`，API 最小日期合同已固定；边界调整后本地自动化 `63/63` 通过。生产继续使用历史 D30 基线，待候选/生产重新验收后切换。
