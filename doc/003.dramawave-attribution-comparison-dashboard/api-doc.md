...
> 状态（2026-08-07）：本文定义目标 D7/D10 合同，尚未切换生产。生产仍运行 2026-08-06 发布的历史 D7/D30 基线。用户已批准 D10 看板起点为 `2026-08-01`，代码和测试边界已同步；下文仍是候选合同，需待生产验收后才能作为线上响应证据。
...
| `start_date` | `YYYY-MM-DD` | 缓存最早日期 | 起始日期，含当天；兼容别名 `start`。不得早于 `/api/meta.minimum_date`；D10 合同固定不得早于 `2026-08-01`。 |
...
响应形状示例（占位时间/版本不是 D10 实测数据；缓存范围以 60 天保留窗口内的真实缓存为准）：
...
`defaults.start_date` 是缓存范围内最近 7 天的起点。它只是页面建议值；其他接口在未传日期时仍默认使用整个缓存可用范围。D10 合同中的 `minimum_date` 固定为批准边界 `2026-08-01`；`expected_start_date` 随 60 天保留窗口滚动，为 `max(2026-08-01, 北京当天-59天)`。
...
- 2026-08-07：目标新口径由 D30 改为 D10；所有 API、排序、质量和导出字段从 `d30_*` 改为 `d10_*`，新源表改为 `kunlunads_dev.ads_app_revenues_10d`。用户批准起点为 `2026-08-01`，API 最小日期合同已固定；边界与 60 天裁剪回归后本地自动化 `64/64` 通过。生产继续使用历史 D30 基线，待候选/生产重新验收后切换。
