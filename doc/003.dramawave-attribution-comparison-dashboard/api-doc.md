...
> 生产状态（2026-08-07）：D7/D10 API 合同已上线，生产 commit 为 `ec6fba1b7e155be904c4c54791fd491c76e09a9a`。最小日期固定为 `2026-08-01`，新归因源为 `kunlunads_dev.ads_app_revenues_10d`。
...
- 2026-08-07：目标新口径由 D30 改为 D10；所有 API、排序、质量和导出字段从 `d30_*` 改为 `d10_*`，新源表改为 `kunlunads_dev.ads_app_revenues_10d`。用户批准起点为 `2026-08-01`，API 最小日期合同固定；自动化、候选缓存、真实数据、生产 API/浏览器和自然 timer 验收后完成生产切换。
