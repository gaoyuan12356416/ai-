...
服务把可用的 `username -> admin_user_id -> email` 映射持久化到本地
SQLite，并每五分钟全量刷新。命中缓存时不查询 MySQL；未命中时按上述
精确路径查询数据库，查到唯一有效映射后立即写入缓存。全量刷新失败时
继续使用最后一次成功缓存，因此服务器重启或只读库短时拥堵不会清空映射。
