...
接口版本：v1 · 文档日期：2026-08-27 · **状态：已上线；真实飞书送达未试发验收**

接口已上线，可在取得本接口专属 Token 和业务授权后调用。上游发送已经汇总的自动复刻事件，接收方可靠接收后尽快异步通知对应剪辑师，不创建或重试复刻任务。

上线代码为 `0a391260f6de1d2e99b351b21d41a613866a5cfb`，2026-08-27 已完成 244/244 项隔离回归和 8/8 项线上非发送检查。没有创建真实测试批次或进行飞书试发；`202` 入队及投递流程由隔离 Mock 回归验证，真实送达仍需首个获准业务批次验收。详见 [部署记录](deploy.md)。
...
全程使用 HTTPS。不要把 Token 放在 URL、请求体、前端页面、截图、日志、群消息或公开代码中。专属 Token 由服务端以 root 所有、0600 权限安全保管，通过安全渠道单独提供，不随文档发送。以下 curl 仅引用安全注入的环境变量；执行真实请求前仍须取得业务授权，不自动向生产发送演示消息。
...
空 `editor_username` 允许接收，直接进入既有兜底群。未找到唯一有效映射、缺少有效邮箱/open_id，或收到确定的私聊失败结果时，使用既有兜底群。消息保留原批次正文，显示请求中收到的 `editor_username`（空值显示为“（空）”）并补充中文原因，不向调用方公开内部群标识。
...
- 本接口已上线；从安全渠道取得专属 Token 和业务授权，真实飞书送达仍需首个获准业务批次验收。
