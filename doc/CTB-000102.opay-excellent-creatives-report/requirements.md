...
北京时间每月 2 日 08:00 启动上一个完整月份的终版刷新，成功后更新公开页面，每月一次。停用原 3 日初版任务，将原 5 日终版任务改到新时间；不再配置随机延迟。页面生成时间仍为实际完成时间。保留终版冻结、失败不覆盖上一成功版本、停机后补执行及既有数据统计规则。

## 2026-08-27 图片/视频全基准与1000美元门槛（最新确认）

用户已确认 2026-08-27 的 7 月只读试算：GG NG40、PK6，共46；包括PK $1.48–$2.04的小额A素材。正式采用 `picvid_cpc_1000_v2`：A累计完整素材USD至全部图片/视频平台USD的50%（跨线/并列），素材CPC < 图片/视频总USD÷总点击，A无金额下限；B素材USD >1000且CTR > 全部图片/视频总点击÷总曝光。NG/PK独立，全部账户国家，未映射资产计入基准。重新刷新1—7月，仅GG；Meta/TT、调度、公开访问与原字段保持不变。
...
- 首次回填 `2026-01` 至最近完整月份；北京时间每月 2 日 08:00 生成上一个完整月份的终版，每月一次。
