...
当前不能启动渲染验收。主任务最新现场记录中，旧正式渲染仍在执行，RSS约 `27,445,544 kB`，主机 `MemAvailable` 约 `4,080,848 kB`；这是2026-08-28的观察记录，执行前必须重新读回，不能当作当前值。旧任务没有因本手册而获得中断授权。
...
| 项目 | 已知记录 / 执行前动作 |
| --- | --- |
| 长片硬链接 | `/data/drama-synthesis-gpu/acceptance/20260828-reliability/inputs/case-679e7c49-concat.mp4`；主任务16:45复核存在，大小 `5139047136` 字节；此前与原concat核对同一inode |
...
当前状态：输入硬链接已保留；源SHA待计算，GPU配方副本待核对，候选最终SHA待填写；**三组短样、90分钟长片、完整解码、RSS/线程判定全部未执行，正式任务排空及本机v1交换保护门禁尚未满足。** 本手册未覆盖真实异步API故障/重启、下载并发/CDN、COS恢复和页面验收，这些仍按主测试报告分别完成。
