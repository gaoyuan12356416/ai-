...
远端窗口继续由迁移任务 `gpu-service-migration-20260828T1502` 协调；历史候选dc0bad8已在一次性书面许可下完成CPU/HK无媒体隔离回归，结果见 [双端无媒体证据](evidence/linux-no-media-regression-dc0bad8-20260831.json)，**该许可已经用完，最终候选a151941的Linux无媒体、HK媒体/COS及部署均须取得新的书面窗口**。旧候选1367dd4的许可和bundle方案均随该SHA作废。无GPU代码测试通过仍不得在HK启动FFmpeg、下载压测、COS写入或真实API重启，也不沿用2026-08-28的旧窗口。CPU公共目录/主API及HK剧集上线必须在发布前再次协调目录、端口和窗口；测试窗口不等同生产切换许可。每次渲染或切换前仍须重新证明正式任务排空，旧PID退出不代表之后不会有新任务。
...
最终运行候选 `a1519413b23d20acab035853b0f5aeebee53e9ac`、tree `2bc83028916e6bc3a6cd7a4cd6cf5f8bc07735ec` 已推送并远端回读，且按Git blob字节冻结 [相对420957b基线的17项code/static运行变更overlay和4个验收工具清单及SHA256](evidence/runtime-file-manifest-a151941.json)。其中相对23a08e5的后续运行增量为10项，相对dc0bad8仍只有 `features/drama_synthesis/gpu.py` 与 `features/drama_synthesis/async_runtime.py` 变化；相对直接父候选83aec118，a151941只改变生产运行文件 `features/drama_synthesis/gpu.py`，4个验收工具blob不变，配套回归测试文件不在运行overlay内。该overlay不是独立部署包，也未穷举候选运行所需的全部未变依赖；CPU/GPU都必须从精确GitHub候选构建完整版本目录，再用清单逐项核对，不得只复制17项。清单不包含私有输入、测试、数据库或生产配置，也不是部署完成证明。历史 [dc0bad8清单](evidence/runtime-file-manifest-dc0bad8.json)、作废的 [1367dd4清单](evidence/runtime-file-manifest-1367dd4.json) 和旧 [570c1bd清单](evidence/runtime-file-manifest-570c1bd.json) 只保留历史证据，不能用于最终候选。后续文档/证据提交不得改变清单内任何运行或验收工具blob；若改变，必须重新生成、回归并选择新的运行候选。
...
| `DRAMA_GPU_RENDER_TIMEOUT` | `43200`秒（12小时），配置允许60～86400 | 专属模板渲染的运维下限；实际初始预算取该值、12小时和 `ceil(整片时长/0.10×1.25+1800秒)` 的最大值并封顶24小时，不再截成10800秒，不绑定HTTP四小时等待 |
| `DRAMA_GPU_RENDER_STALL_SECONDS` | `1800`秒，允许900～7200 | 只有严格增加的FFmpeg媒体时间或帧数才重置；重复fps/speed不能续命；达到阈值后停止自己的子进程并wait确认 |
| 媒体验收unit外层上限 | render `90000`秒；prepare/decode/guard `43200`秒 | render外层比86400秒内部硬上限多1小时，供候选校验、源哈希、诊断和收尾；不得恢复为43200而先于动态预算杀unit。`systemctl show`回读须接受并精确核对等价的 `1d 1h` |
...
| GPU模板失败诊断 | `.runtime/diagnostics/<job_id>/generation-XXXXXXXX.random-render.json` | 0600、原子、每代次隔离；只保存安全错误、预算、进度高水位、退出状态及stderr字节数/SHA/静态标签，不保存stderr原文、命令、路径、URL或凭据；写入失败转人工恢复并保留guard/partial |
...
发布前由目标服务账户预创建并核对工作根、`.runtime/jobs`、`.runtime/locks`、`.runtime/uploads`、`.runtime/diagnostics`、results、manifests和public目录：不得是符号链接，属主/模式须与unit一致；diagnostics根及job目录必须owner-only，sidecar必须0600，工作及结果目录可写，素材目录只读。首次创建目录、诊断sidecar及新manifest的Linux持久顺序必须覆盖父目录fsync；预检应在同一文件系统写入、readback并删除一个非媒体探针文件。该探针只验证权限和持久写路径，不得清理既有账本、诊断、检查点或成片。

concat和去BGM成片在任何publish前先写并readback各自的本地完成记录；恢复时严格校验工作区成片，公开副本缺失只从该成片原子恢复后续传。不得因公开副本/上传响应丢失再次concat或执行Demucs。模板渲染先写start guard，再在输出通过原规格/时长校验、SHA/大小确定且fsync后提交完整prepared；rename或最终记录写入失败时保留并复原。timeout、stall、非零/信号退出及Popen启动失败写入私有诊断；只有进程确认stopped且reader已结束，才清理持久进程身份。alive/unknown或reader未结束时保留进程身份、guard和partial；诊断写失败时即使已清理停止进程的身份，也必须保留guard/partial并进入人工恢复。durable final commit成功后，后续清理异常不得删除或反转已提交结果。有效记录复用和这些保护都不等于长片内存缺陷已关闭。
...
3. 在该SHA的Linux全新隔离目录跑同套测试和真实短子进程跟踪，验证owner/job锁、/proc PID/boot/进程组、Popen启动窗口、动态预算、strict进展、poll二次drain、stall、0600诊断、returncode/signal、reader存活、stopped-only清理、失败清理先partial后guard、成功路径prepared→final持久提交、磁盘失败、停止接新和恢复。最终候选全量为478+FB16；严格无网络按冻结12+3个真实loopback排除后应执行466+13，必须保存实际结果，尚未执行时不得写PASS。测试需满足资源/权限前提，不连接业务发布路由；拉取代码不等于已获GPU制作许可。
...
3. 备份并校验GPU `.runtime`（含独立`uploads`与owner-only `diagnostics`）、完成manifest、render start guard/prepared/final记录和现存产物索引；受限保存含源URL的账本。诊断备份保持目录私有和文件0600，不在日志输出内容；大媒体对象不盲复制耗尽根盘，不删除COS对象或分片。
4. **先停止新写和CPU材料任务worker取新，再等待已经running的唯一制作自然排空。** 冻结并记录running与门禁后queued任务的job ID、输入指纹/原payload哈希、代次、租约和创建时间；queued任务原样保留在门禁后，不解锁、不重提，也不要求删除或消费。若发现GPU仍有活动、未知进程或未对账执行则等待并重新核查，不因发布而切代码/kill进程。另行授权终止需先按第3节处理并确认，本文不授予该权限。业务查询能力保持可用。
5. 当前制作自然结束后先核对GPU完成manifest、本地成片、COS绑定及CPU回填；若失败则保留退出、日志、进程身份、租约和状态证据，不因failed自动重做。排空须证明GPU无running/unknown child、无未对账执行及存活锁；门禁后的queued记录不影响排空判定。
6. GPU从精确候选建立版本化源码目录，沿用已核验依赖、模型和素材。先以隔离根进行语法、导入及运行时预检，确保只读素材与可写work/results权限正确。预检失败不动current。
7. 在GPU空闲后将current原子指向候选源码；只重启 `drama-synthesis-gpu-worker.service`。验证版本、health、认证查询和异步/兼容接口；CPU材料worker仍保持暂停，不能让保留的queued任务被旧同步路径消费。新测试制作只能在已批准隔离环境，不对正式任务POST探测。
8. CPU从同一GitHub精确候选发布，保留已核对的线上差异；新API初始保持 `DRAMA_GPU_ASYNC_ENABLED=0`，只验证业务查询、目录、旧输出预览及权限。**此时不得启动材料worker**，否则门禁后的queued任务可能被旧同步路径领取；不能把新接口上线等同开关开启。
9. GPU兼容接口和隔离全链路通过后，在CPU API与材料worker的同一配置中设置 `DRAMA_GPU_ASYNC_ENABLED=1`。先启动并验证API，再启动材料worker，然后按维护窗口恢复取新，让原queued任务以冻结输入和首个代次自然进入新异步账本；稳定查询且确认GPU只有一个执行后，才恢复新的业务写入口。不制造真实测试任务或发布。
10. 记录每次切换/重启时间、进程健康、首个自然任务的job/指纹/代次、阶段变化、最终产物对账和错误率；提交用户人工检查页面与成片。未获人工验收保持待验状态。

## 6. 切换后检查与停止条件

确认API/worker使用同一开关、同一冻结payload；同job重复查询不会出现第二个渲染进程；CPU更新频率约10秒，页面总耗时持续增长，封面单列完成。同一stage/generation内人工注入迟到的out_time/frame/bytes/percent快照，页面与sidecar仍保持推进高水位而fps/speed可刷新；stage切换后新阶段可重新计数。成片/配方与CPU done一致且完成通知不导致重做。上传重放必须使用相同检查点/UploadId并核对绑定对象元数据，不能只HEAD长度判成功；只有render start guard或未知creating时维持核查，不删记录恢复。回读不得泄露Token、源URL、COS凭据、诊断原文或完整数据库内容。
...
5. **保留CPU数据库及新增表、GPU `.runtime/uploads`与`.runtime/diagnostics`等账本、完成manifest、现存成片、render start guard/prepared/final记录、COS对象和已有分片。** 保持诊断目录owner-only和文件0600，不降级或清空状态，不恢复旧快照覆盖已成功制作的结果；旧代码若不能理解残留未完成账本/诊断，保持材料worker停接新并核查，不abort或新建UploadId绕过旧记录。
6. 记录回滚时间、源码/配置哈希、保留任务清单和产物一致性，再恢复正常入口。单独撤销未经证实的域名/并发/CPU配置，不影响已冻结执行身份。

本次最终候选只完成GitHub推送/远端回读、本地478项与独立终审；没有新Linux执行、真实媒体/COS、真实API或worker重启、远端部署、生产切换、候选参数启用或人工代验收。
