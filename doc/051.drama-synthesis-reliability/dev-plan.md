...
| GPU持久异步运行时 | GPU执行子任务 | async_runtime、GPU HTTP提交/查询/resume、进程/代次栅栏与测试 | 代码已实现；本地及CPU/HK无媒体Linux回归通过，真实接口/重启待验 |
...
| 评审、回归和发布 | 主任务 | 七份合同文档、代码评审、测试报告、部署与回滚证据 | 本地及双端无媒体回归完成；媒体/COS、人工验收和生产切换未完成 |
...
记录命令、代码版本、时间、退出码和数量；每次代码修订后重跑受影响的检查。`git diff --check` 只允许在已信任且已检查repo-local attributes/filter配置的本地开发仓库运行，不属于服务器候选门禁；服务器候选必须走媒体验收launcher的无filter全树验证。最终候选六模块合并回归为 **457/457通过、0跳过**，另有FB隔离 **16/16**；静态列表回归为 **25/25、2页、0次浏览器调用、0次网络调用**。完整回归的真实命令、时长和候选状态由 [test-report.md](test-report.md) 汇总。
...
2026-08-31：dc0bad8完成CPU Python3.9.6与HK Python3.10.20的新私有目录、精确Git身份和严格零网络445+13回归；HK首次缺依赖路径的收集前失败保留，修正仅使用冻结只读依赖目录。未运行媒体/COS或改生产。性能、长片、真实接口重启、生产切换及人工验收仍待独立窗口。
