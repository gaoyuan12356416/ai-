...
5. 演练断言：
   ...
   - 池53 `3CRScaBEY0` 无绑定、无队列/发布历史，错误为 `source_not_repairable`；按新合同校正为 `validation_failed`；
   - 校正后的新分配预览为 10→池2/E8、9→池54/E1、8→池57/E1、7→池60/E1、6→池131/E1、5→池132/E1；
...
7. 使用新 `/internal/posts/drama-pool/check` 精确提交池53、`3CRScaBEY0`、`source_not_repairable` 的既有脱敏错误；入口必须再次断言该剧未绑定且无任何队列历史，结果必须为 `updated_count=1`。
8. 只读确认池53为 `validation_failed`，10:06 原批次仍为 `failed_preflight` 且队列/日志均为 0。
9. 用部署代码在生产在线备份副本上验证新的 6 账号候选映射和完整媒体预检；生产库不建计划、不发帖。
...
- 池53仅发生 `needs_review`→`validation_failed` 的状态校正，仍无绑定、无队列和发布历史。
