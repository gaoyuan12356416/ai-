...
- 未绑定且无历史的新剧确定性校验失败后，标记该剧不可分配并按上传时间倒序继续补位。
- 发布前阻断与归属不一致的旧冻结队列。
- 页面显示持久化“绑定账号”。
...
1. 首次分配按当前配置的账号顺序，将最新上传的未绑定短剧一一绑定。
...
10. 未绑定且没有任何队列/发布历史的新剧发生确定性源数据、剧集或媒体预检失败时，写入 `validation_failed` 和脱敏错误；该剧不绑定账号、不创建队列，当前发布时间点继续按创建时间和池 ID 倒序扫描下一部未绑定新剧。
...
2. sidecar 在 SQLite 快照中先取每个账号未完结绑定，再按账号顺序补充最新上传的未绑定短剧。
3. 选择器只读 `drama_resource`，校验完整免费集并返回每剧下一集。
4. runner 对候选执行确定性剧集与媒体预检；未绑定、无历史候选失败时调用 `/internal/posts/drama-pool/check` 写入 `validation_failed`，重新读取池并继续按上传时间倒序扫描。
...
- `x_post_drama_assignment_conflict`：候选与事务内归属或最新上传优先结果不一致。
...
- 新增 C 后，C 获得最新上传的未绑定剧，A/B 不换剧。
- D1 完成后，A 下一点获得最新上传的未绑定剧。
- 跨账号续发被服务、触发器和发布入口阻断。
- 历史混发记录不被改写，迁移后只有一个确定归属。
- 短剧不足时零队列落库。
- 未绑定、无历史的 D1 确定性校验失败时，D1 写入 `validation_failed`，同账号继续领取倒序中的下一部可用剧；D1 不产生绑定或队列。
...
- 2026-08-04：仅将未绑定新剧的首次分配改为最新上传优先；历史归属判定和已绑定剧继续原账号发布的规则不变。
