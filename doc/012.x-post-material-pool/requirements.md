...
- 短剧预检发现未绑定剧集超过 140 秒时，只能与 token 实时确认具备 Premium 长视频资格账号上的另一条未绑定剧集换位；已绑定连载始终保持原账号。事务仍须使用当前最新未绑定剧集集合和精确集数，Premium 不足或换位后的普通账号候选也超过 140 秒时整批零写入失败，剧集保持未绑定、可重试。
