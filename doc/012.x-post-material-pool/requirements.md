...
- 空池或不足当前目标账号数的合格素材时，整批不创建发布计划、所有账号均不发布。
...
20. 入池即时校验和每次 daily 选材都读取 `ads_drama_info` 中 `app_id=1479`（Dramawave）、同 `content_id + language` 的 `deploy_time`。多端记录取最晚时间；当该时间严格大于本次校验当前时间时，以 `drama_not_yet_deliverable` 暂时跳过并继续扫描后续素材。时间相等或已经过去时恢复候选资格；`deploy_time=0` 表示无未来时间限制。映射缺失或时间非法时 fail closed。该错误不形成 queue 绑定，后续 daily 仍会重新读取，所以素材到时后可自动参与发布。
...
- 仅在操作员明确授权后，允许恢复北京时间当天、原状态为 `failed_preflight`、且错误精确为 `x_token_missing` 或 `x_upstream_error` 的冻结 schedule run。
- 恢复前必须证明 run 的预期数与冻结账号数相等、四个结果计数均为零、没有 queue/log、没有 unknown/post_creating、冻结随机计划/固定配置仍精确匹配，且全部冻结账号当前为 active、已批准发布并存在 Token 引用。
- 原失败状态、错误、起止时间、操作人和固定策略 `operator_same_day_compensation_v1` 先写入唯一审计表，再把同一冻结 run 原子恢复为 `claimed`。每个 run 只允许恢复一次。
- 恢复工具只改为待处理，不选素材、不建 queue、也不调用 X；由原 schedule timer 在共享锁下重新预检并顺序发布。跨日、已有写入、错误不匹配或审计已存在时一律拒绝。
- 若首次同日恢复后仍因已确认并修复的发布前缺陷再次零写入失败，只允许使用 `operator_same_day_corrective_retry_v1` 再纠正一次；必须存在首次恢复审计、错误码和错误消息命中显式白名单、仍无 queue/log/unknown/post_creating，并写入独立唯一审计表。不能删除或覆盖首次恢复历史。
- 若该纠正重试又在 Premium 换位中遇到 `x_post_media_repair_invalid_response`，只有在 GPU manifest 为 ready、输出哈希/大小/探测均已复核且 CPU 严格客户端对同一幂等 job 读回通过后，才允许使用 `operator_same_day_verified_repair_retry_v1` 再恢复一次。必须记录 64 位 repair job key、关联前两级审计并保持零写入；每个 run 只允许一次，不能扩展到其他错误。
...
- schedule 媒体下载对 `media_download_failed` 最多执行 3 次完整、独立的有界尝试；其他错误不重试，下载临时文件仍由严格下载器逐次清理。若已使用账号刷新审计的同一 run 随后精确因瞬时 `media_download_failed` 零写入失败，只有在该重试修复 commit 已部署后，才允许使用 `operator_same_day_transient_media_retry_v1` 再恢复一次，并关联前一级审计、记录 40 位 commit 且保持单次约束。
...
| 2026-08-11 | 增加操作员授权的同日零写入 failed_preflight 审计恢复工具；不改写 queue/log 历史，不允许跨日或重复恢复 |
| 2026-08-11 | 修正调度客户端对素材池顺序的反向判定；与素材池及建队列统一为 `created_at,id` 倒序 |
| 2026-08-11 | 修正 Premium/普通账号能力路由后的素材池一致性校验；保留“最新 N 条”集合约束，允许集合内按账号顺序重排 |
| 2026-08-11 | 增加首次恢复后的单次纠正重试审计，并允许未绑定短剧在 token-confirmed Premium 与普通账号之间安全换位；已绑定短剧账号归属不变 |
