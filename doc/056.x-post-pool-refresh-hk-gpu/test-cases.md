...
| TC-006 | 香港 worker 启动 | Python/COS/ffmpeg 配置完成 | 检查本机 health 与错误 token | 正确 token health，错误 token 被拒绝 | P0 | 待生产 |
| TC-007 | NVENC/COS 回填 | 8 个显式素材 ID | 运行 backfill | 逐条修复/验证并更新池状态，零队列 | P0 | 待生产 |
| TC-008 | 历史状态刷新 | 冻结指纹未漂移 | 当前源/账号能力确认后事务更新 | 212 条清空，无越界 | P0 | 待生产 |
| TC-009 | 精确删除 | 3 条未发布无占用 | 在备份后事务删除 | 仅 ID 86/296/297 被删 | P0 | 待生产 |
| TC-010 | deferred 保护 | 5 条未到投放时间 | 前后对账 | 仍为 deferred 且数量为 5 | P0 | 待生产 |
| TC-011 | 零真实发布 | 记录队列/日志基线 | 完成全部验收后复核 | 队列与发布日志未增加 | P0 | 待生产 |
