...
管理员在 `/x-account-list.html` 按账号查看 X 运营产出和收入；页面打开时不得扫描 MySQL。统计在北京时间 10:00、11:00、12:00 刷新到数据盘缓存，以覆盖昨日收入的延迟入库。
...
- 页面请求不连接 MySQL；OAuth Sidecar 文件零改动；timer 为 10:00/11:00/12:00（Asia/Shanghai）。
