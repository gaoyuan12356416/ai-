...
回滚：同一release下python3 scripts/deploy_youtube_analytics.py --rollback <backup>。校验当前SHA与部署manifest一致，拒绝覆盖后续变动；只恢复代码与导航，保留当前数据库、短链、资产、发布/预约及投递事实。无需数据库迁移或新timer。公网需要新增deploy/youtube-analytics.nginx.conf，通过scripts/deploy_youtube_analytics_proxy.py安装并nginx -t后平滑reload。

验收需要匿名401、真实会话只读报表/筛选/export、公共静态字节匹配、worker PID保持及API健康。具体commit/backup/检查结果后续写入deployment-evidence.md。

代理单独备份，完整回滚先运行 `python3 <proxy-release>/scripts/deploy_youtube_analytics_proxy.py --rollback <proxy-backup>`，再运行主代码回滚。两者均拒绝覆盖后续变动；精确路径见deployment-evidence.md。
