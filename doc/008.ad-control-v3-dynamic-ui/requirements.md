...
两页均要求 cookie 登录和独立的 `ad_control_v3` 模块权限；管理员可在用户管理中单独授权，旧版继续使用 `ad_control_center`。对历史上已获得旧版权限且尚未保存 V3 独立权限的用户，兼容继承原值，避免上线瞬间丢失访问。新页面固定显示：旧版独立运行、本期只支持保存草稿与手动试算、调度器未发布、启用锁定。
