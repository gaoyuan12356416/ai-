...
V3 用户支持多个强身份优化师别名；临时管理员选择本人任一别名时同样扩展本人完整集合，选择他人仍保持单优化师。规则组按关系表保存，扫描按别名逐一查询并合并。
...
6. 使用王鹏和凌云柯当前有效会话调用 meta 和规则列表，仅输出脱敏后的状态/优化师 ID。
...
- 凌云柯 `/meta` 返回 `[404,687]`，规则列表返回 200。
...
- 若王鹏身份不是严格 `[387,686]`，或凌云柯身份不是严格 `[404,687]`，立即回滚增量代码并保留数据库审计表。
