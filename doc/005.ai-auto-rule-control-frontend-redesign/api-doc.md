...
后续正式实现优先复用：
- `GET /api/ad-control/products?families=dramawave,hotdrama,freereels`
...
## 请求/响应
计划变更：
- 产品列表增加产品线过滤参数，默认仅返回 `dramawave`、`hotdrama`、`freereels` 对应产品。
...
## 兼容性说明
新前端必须兼容现有数据模型，不破坏旧页面缓存和旧 API。若后端暂未支持 `families` 参数，前端可以先基于返回字段做保守过滤，但最终以后端过滤为准。
