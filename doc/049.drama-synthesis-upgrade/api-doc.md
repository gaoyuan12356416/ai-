...
2026-08-27 最新增量不改变 HTTP 请求合同：结果 ledger 为 ads_ai 专用新表，原库继续只读频道/OAuth；账户 JSON 查询以 HEX 安全传输，不修改原值。受控 RPC health 为 v3/ads_ai，诚实声明 shared-existing-account / application-table-allowlist / db_least_privilege=false，旧版拒绝；见 [新表合同](ads-ai-new-tables-20260827.md)。
