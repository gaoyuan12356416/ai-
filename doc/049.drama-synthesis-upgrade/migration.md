...
## 最新执行入口：只在 ads_ai 新建三表

按用户最新指令，现行建表与证据步骤以 [ads_ai 新表发布合同](ads-ai-new-tables-20260827.md) 为准。旧库只读，不复制原数据、不 ALTER 原表；新 bootstrap 与 v2 runtime 分离。以下旧迁移/c719 snapshot 仅作历史证据，不再执行，旧库写权限不再是阻塞。
