...
| 单元与契约测试 | Codex | `ops/ai-game-performance/test_*.py` | 本地/服务器 20/20 通过 |
...
5. 待完成：服务器只读阴影刷新与 MySQL/SQLite/JSON 对账；不得在验证前切换 `latest.json/current`。
6. 按用户本次“补一下”的发布授权执行：GitHub-first 不可变 release、生产前备份、原子切换、浏览器验收和自然 timer 观察。

当前生产仍不包含 Unity 补数；本节不构成部署记录或发布批准。
