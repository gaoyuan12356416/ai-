...
| TC-006 | 分片恢复 | 第 2 片失败 | 重启执行 | 第 1 片不重制，第 2 片继续 | P0 | 自动及正式任务受控恢复通过 |
| TC-007 | 身份冲突 | 修改源或 scene | 重放 | fail closed，不覆盖旧片 | P0 | 自动通过 |
| TC-008 | 连续音频 | 多分片、有/无音轨 | 合并和 mux | 单音轨、无接缝、时长误差≤0.15秒 | P1 | 自动及正式全长完整解码通过 |
...
| TC-011 | full-bleed clean 几何 | 横/竖源、scale 0.98～1.02、rotation ±2° | 编译/真机渲染 | 主体 cover；仅外缘可透明，不含 contain 横向边界或 legacy 错误画布/偶数偏移 | P0 | 自动及横竖真机通过 |
| TC-012 | T4 代表性视觉 | 竖屏、横屏、混合分辨率真实素材及冻结配方 | clean CPU 参考/V2 双渲染并导出单画面预览 | 无黑块、横向断层、错位裁切或分片跳帧；构图、素材、旋转和时长符合合同 | P0 | 真机与正式成片抽检通过 |
| TC-013 | T4 长样性能 | 约79.4分钟同源素材 | V2 基准 | 完整完成；采用同机最快稳定配置；无 swap、资源有界 | P0 | 300秒基准及92.9分钟正式任务通过 |
