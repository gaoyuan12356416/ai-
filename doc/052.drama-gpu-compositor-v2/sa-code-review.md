...
| CR-005 | P1 | 并发 | 两个完整任务会争抢 Demucs、显存和收尾缓存 | V2 固定完整任务 1；仅同一任务内部允许 2 个 chunk lane | 已修复，双 lane 待真机 |
| CR-006 | P2 | 部署文档 | compositor cache 路径写成 jobs | 改为 `work/compositor-cache` | 已修复 |

## 编译 / 验证结果

本地 Python 3.9 AST 校验通过；497 项相关完整回归通过，6 项按既有平台条件跳过。真机结果记录到测试报告。
