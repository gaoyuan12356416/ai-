...
| `4cbd8fe` | 300 秒 | 上限 4（实际 3 块并行）、2 threads/lane | 输出契约通过 | 1.300× | 1883 MiB | 0 |
| `4cbd8fe` | 300 秒 | 2 lane、2 threads/lane | 输出契约通过 | **1.388×** | 1263 MiB | 0 |

因此生产默认选 `2 lane × 2 threads`；保留 1～4 lane 泛化能力，但不把最大并发误当成最高吞吐。最终 exact release 的视觉对照、受控恢复、79.4 分钟长样与生产切换，因同机另一已授权迁移窗口正在执行而暂停，未绕过验收。
