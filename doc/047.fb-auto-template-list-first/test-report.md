...
本地完整回归、浏览器 QA、旧页面缓存回归和生产只读验收全部通过；列表优先页面及缓存修复已完成受控发布。
...
第一轮代码评审发现确认框监听清理问题，已在完整 QA 前修复。首次生产发布后，运营截图暴露浏览器恢复旧合并 HTML 的 P0 缓存缺陷 `BUG-001`；已通过版本化导航与精确 `no-store` 响应头修复并关闭。

## 验证证据

- `node --check static/quick-nav.js static/fb-auto-publish-{common,templates,template}.js`：4/4 通过。
- `python -m unittest scripts.test_fb_auto_app_contract`：Ran 6 tests，OK。
- `python -m unittest discover -s scripts -p "test_fb_auto_*.py"`：Ran 81 tests，OK。
...
- 缓存修复提交 `490e3b78cd418e0114e2abba7b097653f18e47b0`：版本化导航、列表→独立创建页、编辑和保存返回契约通过；本地 Playwright 控制台 0 error/0 warning。
- 缓存修复生产验收：无参数及版本化的列表、表单、记录 HTML 均 HTTP 200 且含 `no-store`；公网导航和 QuickNav 指向版本化列表，列表无 `templateForm`，匿名模板 API 保持 401。
- 缓存修复发布后：`live_enabled=false`；sidecar `MainPID=3083645`、Nginx `MainPID=2164`，二者 `NRestarts=0`、`active/running`；六张运行表仍为 0，`PRAGMA quick_check=ok`。
- 全程未创建或修改生产模板，未调用 enable/disable/run-now，未产生 Graph Post；未重启主 API 或 FB sidecar，仅在 `nginx -t` 通过后 reload Nginx。
