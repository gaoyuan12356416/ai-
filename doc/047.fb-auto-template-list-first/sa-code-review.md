...
| CR-05 | P0 | HTML 缓存 | 首次发布只有 ETag/页面元信息，旧合并页仍可能被浏览器恢复 | 版本化全部关键跳转并新增三个精确 Nginx `no-store` location | 已修复并通过生产验收 |

## 编译 / 验证结果

- 三个新增 JavaScript：`node --check` 通过。
- `python -m unittest scripts.test_fb_auto_app_contract`：6/6 通过。
- FB 全量回归 81/81、X/TT 主契约 66/66 通过。
...
- 缓存修复浏览器回归：版本化导航进入纯列表页，列表 DOM 无 `templateForm`；点击“创建模板”进入独立表单页，返回链接保持版本化；控制台 0 error/0 warning。
- 缓存修复生产验收：无参数及版本化的列表/表单/记录 HTML 均为 HTTP 200，并返回 `Cache-Control: no-cache, no-store, must-revalidate, max-age=0`、`Pragma: no-cache`、`Expires: 0`；匿名模板 API 保持 401。
