...
| --- | --- | --- | --- | --- | --- | --- |
| TC-001 | 增量建表 | 仅有旧 X 账号库 | 两次调用 `ensure_storage` | 两张新表/索引存在，旧表行与 Token 不变 | P0 | 本地通过；生产副本待验收 |
...
| TC-005 | 帖子正文 | 长 URL 与长中文描述 | 生成文本 | 第一行短链保持完整，描述保守截断且非空 | P0 | 通过 |
...
| TC-010 | Create Post 成功 | media ID 可用 | POST `/2/tweets` | 文本与 media ID 正确，保存 post ID/预览链接 | P0 | mock 通过；真实待验收 |
...
| TC-016 | 日志脱敏 | 上游错误含敏感字段 | 查询 DB/journal | Token、Authorization、素材鉴权查询串无命中 | P0 | 本地通过；生产 journal 待验收 |
| TC-017 | 真实候选合规门禁 | 生产只读数据 | 审计 top candidates | 所选素材所有违规和色情/暴力标签命中均为 0 | P0 | 通过：5221348 |
| TC-018 | 真实短链与 Post | 用户已授权灰度 | 发布一次并公网访问 | 短链/落地页/X status 均可访问，日志 published | P0 | 待执行 |
