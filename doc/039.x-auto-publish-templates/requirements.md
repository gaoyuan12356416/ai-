...
- 模板选择一个明确的剧语言，并可选择多个当前 `status=active`、`publish_approved=true` 且 `publish_eligible=true` 的 X 账号；`refresh_required` 账号必须由有模板导航权限的操作员显式刷新成功后才可勾选。
- 编辑页为符合条件的 `refresh_required + publish_approved=true` 账号提供“刷新可选账号资格”入口；页面在一次用户操作中逐账号串行调用精确刷新接口，账号列表读取本身保持只读，不调用 X。
...
1. `GET /accounts` 只读取现有 X sidecar 的安全账号快照，不调用 X、不刷新或改写 Token。
2. 只有具备 `xAutoPublishTemplates` 导航权限的登录操作员可以刷新账号资格；页面仅收集当前 `refresh_required` 且 `publish_approved=true` 的账号，并逐个串行刷新，不存在服务端批量刷新接口。
3. 刷新成功后必须回读到同一账号身份，且达到 `status=active`、`publish_approved=true`、`publish_eligible=true`，页面才解除复选框禁用；未批准发布的账号即使凭证可刷新也仍不可刷新、不可选择。
4. X 网络超时、限流或临时上游错误不得把账号降级为永久失败；账号保持 `refresh_required` 并允许操作员稍后重试。若 X 明确返回 Token 已撤销/失效，则账号进入撤销状态并提示重新授权，不允许继续刷新或选择。
5. 显式刷新只解决当前账号资格，不等于选择模板、启用模板或授权发帖。创建、编辑、启用、立即执行、scheduler 建任务和最终发布仍沿用现有严格账号校验，任何一步发现非 `active + approved + publish_eligible` 都失败关闭。
...
3. 编辑页包含模板名称、语言、账号、正文、统计窗口、剧规则、素材规则、冷却、固定/随机计划；账号区展示实时快照状态，并提供逐账号串行的刷新操作，刷新成功前复选框保持禁用。
...
- 账号列表 GET 不调用 X；页面只会逐个刷新 `refresh_required + publish_approved=true` 账号，刷新成功后才可选择，未批准账号保持不可选。
- 临时刷新错误保留 `refresh_required` 可重试，明确撤销要求重新授权；刷新失败不得创建模板、run、task、queue、log 或 Post。
- 创建、编辑、启用、执行与最终发布的既有严格账号校验不得因新增刷新入口而放宽。
...
- 2026-08-12：增加有模板导航权限的操作员显式刷新资格入口；列表读取保持只读，账号刷新与模板选择、运行和最终发布严格分层。
- 2026-08-12：预览和实际选择增加账号会员感知的有效时长范围；标准账号最多 140 秒，只有 token 确认的 Basic/Premium/Premium+ 账号可在模板范围内选择 140–600 秒素材。

## 账号会员感知的素材时长

1. 模板的 `duration_min_seconds/duration_max_seconds` 是业务筛选范围，600 秒仍是
   X Auto 产品硬上限；它不能覆盖账号自己的发布资格。
2. 预览使用本次验证得到的账号快照，实际任务选择使用运行创建时冻结的账号快照，
   把标准账号有效上限收紧到 140 秒。
3. 只有 token 同时报告 `long_video_eligible=true` 且 `subscription_type` 为
   `basic`、`premium` 或 `premium_plus` 时，账号才可在模板范围内选择
   140–600 秒素材。公开蓝标或单独布尔值均不能作为资格。
4. 最终媒体准备和发布前继续重新验证账号；会员降级必须失败关闭，不得换号或放宽
   时长。模板最小时长高于账号有效上限时应得到 `no_candidate`。
