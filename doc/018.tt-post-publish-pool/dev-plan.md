...
| 任务 | 负责人 | 文件/模块 | 状态 |
| --- | --- | --- | --- |
| 需求、规则和 API 设计 | PM/SA | `doc/018.tt-post-publish-pool/` | 增量已更新 |
| CPU SQLite 状态机与安全账号源 | 开发 | `features/tt_posts/` | 上一基线已完成；增量实现中 |
| GPU 成片与 TikTok sidecar | 开发 | `features/tt_gpu/`、`scripts/tt_gpu_worker.py` | 上一基线已完成；本轮语义不变 |
| 主后台路由、权限和审计 | 开发 | `app.py` | 上一基线已完成；本轮不新增路由 |
| 发布池 UI 与导航 | 前端 | `static/tt-post-pool.html`、导航配置 | 上一基线已完成；增量实现中 |
| claim/runner、systemd 与隧道 | 开发 | `scripts/`、`deploy/` | 上一基线已完成；本轮语义不变 |
| 单元、合同、浏览器与线上关闭态验收 | QA/SA | `scripts/test_tt_*.py` | 上一基线已完成；增量待执行 |
| GitHub-first CPU/GPU 部署 | 运维 | 两台服务器 immutable release | 上一基线已完成；增量待部署 |

### 本轮增量任务

| 任务 | 文件/模块 | 完成条件 | 状态 |
| --- | --- | --- | --- |
| 批量素材 UI | `static/tt-post-pool.html` | X 同款分隔/去重、1–100、逐条 preview、结果列表、单项失败继续 | 实现中 |
| 批量排期 UI | `static/tt-post-pool.html` | 首条上海时间、1–1440 分钟间隔、默认 10 分钟、逐条 queue、失败项不影响后续 | 实现中 |
| 可编辑描述模板 | UI、`features/tt_posts/` | 默认模板首屏显示；占位符校验；每素材真实 ID 渲染；模板和最终文案冻结 | 实现中 |
| prepare 复用 | `features/tt_posts/service.py`、GPU 合同测试 | preview 与 queue 对同一素材身份使用同一确定性 job，源/profile 变化时失效 | 实现中 |
| 幂等与历史兼容 | Core、Service | 模板/文案纳入冲突比较；旧 `caption_text`、缺省模板及历史任务精确重放 | 实现中 |
| 自动化与浏览器验收 | `scripts/test_tt_*.py`、生产关闭态页面 | 新旧回归通过；混合失败、同时间冲突、多任务重试和历史重放通过 | 待执行 |
| GitHub-first CPU 部署 | CPU immutable release | 备份、切换、健康检查和公网浏览器验证；不创建真实 Post | 待执行 |

本轮不修改 GPU release 的媒体制作语义，不新增 TT 批量表；GPU 是否需要切换 release 以最终代码差异为准。
...
- 最多 100 个素材的 preview/queue 为顺序请求，整体耗时可能较长；页面必须持续显示进度，且某项失败不能中断循环。
...
- 2026-07-29：完成批量素材、时间间隔、可编辑描述、部分失败、prepare 复用和历史兼容的增量设计；代码验证、部署与生产验收待执行。
