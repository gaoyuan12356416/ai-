...
5. 用户必须看到目标账号昵称、每个成功素材的最终视频及描述规则，并显式确认音乐条款和商业内容披露，才可排期。一次确认作用于本批成功素材，但每条任务仍分别冻结设置。
...
6. 用户选择隐私/互动/商业声明，编辑描述模板，选择首条北京时间和后续发布间隔并确认预览和同意条款。间隔为 1–1440 分钟的整数，默认 10 分钟。
...
- 精确重放旧任务不得改写历史模板或描述，也不得再次调用 GPU。相同幂等键只要素材、账号、时间、模板、渲染文案或冻结设置任一改变，就返回幂等冲突。
...
9. 未选择隐私、未完成显式同意、模板缺少 Drama ID 占位符、含未知占位符或任一最终描述超长时不能排期。
