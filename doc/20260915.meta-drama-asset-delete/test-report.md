...
## 后台规范复核
2026-09-15追加UI修正：去除页面对共享导航/用户卡的自定义覆盖，恢复深色侧栏、菜单说明、标准用户卡和按钮；共享顶栏CSS放在页面CSS之后。修改仅HTML/CSS。

- 浏览器本机Mock确认标准侧栏、用户卡、登录按钮及刷新按钮；默认无选中产品。
- 进入/刷新仅展开Meta广告清理；手动收起有效。展开工作台后任务列表链接为根路径 `/drama-synthesis.html`，不会落在本页hash。
- 使用真实共享QuickNav/UiTopbar文件验证，JS语法检查通过。
- 技能要求的8个Python入口py_compile通过；独立模块已在后端模块表、普通用户默认权限及执行校验中接入。
- 本轮没有新增删除或数据库写测试，原106项业务测试结论及Video限制保持原有适用范围。
