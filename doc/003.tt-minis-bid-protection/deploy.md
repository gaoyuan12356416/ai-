...
- 新增独立 TT 小程序 Bid Protection 同步脚本和北京时间每日 `09:25` root cron。
...
- 时区：CPU 主机固定 `Asia/Shanghai`，每日 `09:25`。
...
7. 手工执行 `python3 tt_minis_bid_protection_sync.py --backfill-days 60`。回填可重跑，按日/账户/层级分批，任何失败不得清空成功行。
8. 创建权限受限的独立日志目录。完成重复运行幂等、三产品覆盖、两个层级、金额缩放和 DramaWaveMinis `2026-09-02` 样本验收后，安装以下每日 cron 并读回确认：
...
- 首次 60 天回填需人工执行且观察 API 限流；cron 后续每日只运行一次。
