...
本地阶段通过，生产基础部署通过，数据验收仍在进行中。24 项自动化测试、生产 DDL 读回、精确 release 发布和 572 账户 Token 全量兼容 canary 已通过；60 天回填、幂等、最终覆盖、cron 与样本核对尚未执行，因此当前不作完整发布通过声明。
...
- `python -m unittest discover -s ops/tt-minis-bid-protection -p 'test_*.py'`：24 项全部通过。
