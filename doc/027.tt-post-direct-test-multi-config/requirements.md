...
6. 开启或重新开启配置时需要非空账号列表、已有发布设置和有效确认；账号当前状态、实时能力和三重门禁在真正执行发布时重新校验，不阻断配置保存。
7. 关闭时允许在账号源不可用、账号已失效或门禁关闭的情况下执行“纯关闭/移除”：保留当前模板和时间，只能保留或移除既有成员，不得新增未知账号；不要求新的 creator-info、门禁或用户确认。
8. 修改模板或新增账号时，模板仍需通过宏/UTF-16 校验，新增账号必须已有本地发布设置；保存不依赖当前账号快照，真正发布时再次完成实时校验。
...
- 2026-08-03：BUG-005 已上线并关闭：发布任务主表统一显示 queue/direct-test；新增只读 `/tasks`，旧 `/queue` 保持；direct 合成行不提供 queue 事件或写操作。代码、专项自动化、全量回归、服务器候选测试和生产只读验收均已完成。
