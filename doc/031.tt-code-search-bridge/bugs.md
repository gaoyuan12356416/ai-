...
多轮独立代码评审发现的问题已在当前工作树修订，并通过最终 394 项全量回归和独立收口复审；当前没有未关闭的 P0/P1/P2。以下“已关闭”仍仅表示代码与本地验证完成，不能当作生产线上验证。
...
| BUG-007 | P1 | Redis data dir 启动前不存在 | 独立 unit 可能因目录/权限失败 | `ExecStartPre` 创建数据盘目录并设 `tt-post` 0700 | 已关闭，待候选验证 |
