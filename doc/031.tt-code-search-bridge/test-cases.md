...
用例已设计、待代码完成后执行。除明确标注“生产只读”的项目外，全部使用临时 SQLite、fake resolver、fake Redis 和隔离浏览器；不得连接 TikTok publish/canary 接口。
...
| queue 101 / 102 | 最新 published 排序与并列时间兜底 |
...
| 编号 | 场景 | 前置条件 / 步骤 | 预期结果 | 优先级 | 状态 |
| --- | --- | --- | --- | --- | --- |
| C01 | 空库迁移 | 对临时旧版 DB 执行 ensure storage | 加法创建 `tt_post_code_route` 和索引，旧表/行不变 | P0 | 待执行 |
| C02 | 重复迁移 | 连续执行迁移两次 | 幂等成功，schema/数据一致 | P0 | 待执行 |
| C03 | code 格式约束 | 插入小写、3/5 位、符号、Unicode | DB CHECK/业务校验拒绝 | P0 | 待执行 |
| C04 | 正常分配 | 新 queue freeze | 产生 `^[A-Z0-9]{4}$` code，route 与 caption 同值 | P0 | 待执行 |
...
| C14 | 回收审计 | 发生满容量回收 | 记录旧 code/queue 与新 queue 的安全审计，不含敏感数据 | P1 | 待执行 |
...
| D09 | 历史模板 | 模板不含 `{code}` | 既有渲染行为和错误码不变 | P0 | 待执行 |
...
| E02 | `c` 尾部身份 | code=AB12、queue=101、短链 ID 另值 | `c` 尾部为 101，不是 AB12/短链/publish_id | P0 | 待执行 |
...
| F03 | 未知 code | 无 AB12 | 404 `found=false` | P0 | 待执行 |
| F04 | 直接 ID clone | 同剧有多条 published | 选最新 `published_at,queue_id`，仅 channel 改 Search | P0 | 待执行 |
| F05 | published 时间并列 | 两条同 `published_at` | 选 queue ID 较大的最新行 | P0 | 待执行 |
...
| F13 | 剧不存在 | route 指向的 content 已无法由剧 resolver 确认 | 404 或已冻结的明确产品语义；必须与 API 文档一致且无 CTA | P0 | 待执行 |
| F14 | 上游异常 | 剧 resolver / SQLite 超时 | 503，不伪装 404 | P0 | 待执行 |
...
| 编号 | 场景 | 前置条件 / 步骤 | 预期结果 | 优先级 | 状态 |
| --- | --- | --- | --- | --- | --- |
| G01 | cache hit | Redis 有合法且当前的 published DTO | 返回正确路由，标记 HIT | P1 | 待执行 |
| G02 | cache miss | Redis 无 key，SQLite 有 route | SQLite 返回并安全回填，标记 MISS | P0 | 待执行 |
| G03 | Redis 停止 | 6381 拒绝连接 | SQLite 仍返回正确 found/404，标记 BYPASS | P0 | 待执行 |
...
| G07 | 回收陈旧值 | Redis 有旧 AB12，SQLite 已回收为新 route | 绝不返回旧 target；`DEL` 失败时旋转 namespace 并旁路读 SQLite | P0 | 待执行 |
| G08 | 写缓存失败 | SQLite commit 成功、Redis SET/DEL 失败 | 发布事实保留，旋转 namespace/旁路 SQLite，无 500/数据回滚 | P0 | 待执行 |
