...
代码、最终全量 TT 回归、独立复审、生产候选迁移、Redis/Nginx/systemd 部署和线上验收均已完成；当前没有未关闭的 P0/P1/P2。生产后端 release 仍为 `b01dabe22d9da1571c68b6fb0775a61bb48e18de`；`/tt-code` 已叠加静态热修 commit `20ceee877541ea10b11e4a63420ffdbca43d4cec`。
...
2026-08-05 Featured 点击热修证据：

- 真实生产复现为 `pointerdown` 在卡片内部、`pointerup/click` 被容器 capture 改投到 `#stories`，因此 resolver 请求为 0；后端五条 Featured 和五条 resolve 均正常。
- 最终本地 exact diff：Python 395/395；新/旧 Node bridge 89/53 assertions；`node --check`、`git diff --check` 和原 `/tt` 零 diff 均通过。
- 独立终审先后发现并关闭阈值前出界松手、意外 `lostpointercapture` 两个边界；最终 P0/P1/P2 均为 0。
- 真实 Chromium：普通点击与 4px 轻移各发 1 次 resolver 并跳转；100px 横拖发 0 次并滚动到下一卡；阈值前出界松手和强制丢 capture 均发 0 次、无残留拖动。
- 生产 390x844 与 1440x900 均恰好 5 卡，点击第一卡各发 1 次 resolver，落到 `af_dp=ZZ4b4w5k3h&af_channel=Featured`；拖动和出界用例均 0 resolver，console/page error 均为 0。
...
| Node bridge | 通过：当前热修新 89、旧 53 assertions | exact commit 候选复跑通过 |
...
2026-08-05 静态热修后，后端 current symlink 仍指向上述 release；公开 `/tt-code` JS 精确来自 `20ceee877541ea10b11e4a63420ffdbca43d4cec`，未重启服务、未改数据库、未触发发布。

## 缺陷情况

独立评审确认并修复了历史 pending queue、AIpost 默认值、公共 sidecar 暴露、Redis 共享锁、高占用 SQL、URL 顺序、Redis data dir、迁移原子性、回收审计、`{code}` exact retry 误冲突、输入变化后的旧 CTA/race，以及 Featured 鼠标点击被 carousel capture 吞掉的问题；详见 `bugs.md` 与 `sa-code-review.md`。最终全量与独立终审均通过，当前无已知未关闭 P0/P1/P2。
...
2026-08-05 热修线上证据：公开 JS SHA-256 为 `e3063f9a872cfa07eb56f1b9ea54751b96532508244893c67b15b70672db2ba0` 且 `Cache-Control: no-store`；旧 `/tt` HTML/JS SHA-256 仍为 `abce427...` / `635d50...`。热修前后生产自然账本均为 queue `19`、max queue `19`、publish IDs `18`、schedule runs `19`、random plans `14`、route `12`、recycle audit `0`，`integrity_check=ok`。
...
- 生产已有 Redis、新 Nginx、新静态和 DB schema 证据；2026-08-05 热修时 route 为 12、audit 为 0，均为既有自然生产记录；验收未伪造 route、未触发发布。
