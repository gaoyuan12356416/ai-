(function () {
  ...
  function manualRunStorageKey(id, version) {
    return `tt-auto-run-now:${id}:v${version}`;
  }
  ...
  function renderTemplates() {
    ...
    state.templates.forEach(item => {
      ...
      const languageSummary = item.language_summary || item.drama_language_summary;
  ...
  async function mutateTemplate(item, action) {
    ...
    try {
      ...
      if (action === "copy") {
        ...
        if (copiedId) {
          location.href = `/tt-auto-publish-template.html?id=${encodeURIComponent(copiedId)}`;
          return;
        }
      } else {
  ...
  async function handleTableAction(event) {
    ...
    if (action === "edit") {
      location.href = `/tt-auto-publish-template.html?id=${encodeURIComponent(id)}`;
      return;
    }
    if (action === "runs") {
