(function () {
  ...
  const API_BASE = "/api/admin/tt-auto-publish";
  let toastTimer = 0;
  ...
  async function initShell(options) {
    ...
    await window.QuickNav.render({
      container: "#quickNav",
      auth,
      activeKey: config.activeKey || "ttAutoPublishTemplates",
    });
