(function (root) {
  ...
  const FEATURED_LANGUAGE_SCHEMA_VERSION = 3;
  ...
  const FEATURED_TIMEOUT_MS = 4000;
  ...
  const FEATURED_THUMBNAIL_PATTERN =
    /^\/tt-featured-covers\/[a-f0-9]{64}\.webp$/;
  ...
  function featuredLanguageForLocale(locale) {
    const selected = normalizeLanguageTag(locale);
    if (!Object.prototype.hasOwnProperty.call(COPY, selected)) {
      return "en";
    }
    // The spend source publishes one verified Chinese ranking bucket. Keep
    // Simplified Chinese UI copy while selecting that shared ranking.
    return selected === "zh-hans" ? "zh-tw" : selected;
  }
  ...
  function buildFeaturedUrl(language, origin) {
    const selected = normalizeLanguageTag(language);
    if (!selected || !FEATURED_BUCKET_LANGUAGE_PATTERN.test(selected)) {
      throw new TypeError("Invalid featured language");
    }
    return new URL(
      FEATURED_PATH + "/" + selected + ".json",
      normalizeOrigin(origin)
    ).href;
  }

  function buildFeaturedBundleUrl(origin) {
  ...
  function isSafeFeaturedThumbnail(value) {
    return FEATURED_THUMBNAIL_PATTERN.test(String(value || ""));
  }
  ...
  function normalizeFeaturedLanguagePayload(
    payload,
    expectedLanguage,
    nowMs
  ) {
    const language = normalizeLanguageTag(expectedLanguage);
    const topLevelKeys = [
      "schema_version",
      "source_date",
      "generated_at",
    ...
    if (
      !language ||
      !FEATURED_BUCKET_LANGUAGE_PATTERN.test(language) ||
      !hasExactKeys(payload, topLevelKeys) ||
      payload.schema_version !== FEATURED_LANGUAGE_SCHEMA_VERSION ||
      payload.language !== language ||
      !Array.isArray(payload.items) ||
      payload.items.length !== 5 ||
      containsPrivateSpendKey(payload)
    ) {
      throw new TypeError("Invalid featured language payload");
    }
    ...
    for (const source of payload.items) {
      ...
      if (
        !source ||
        requiredItemKeys.some(function (key) {
          return !Object.prototype.hasOwnProperty.call(source, key);
        }) ||
        keys.some(function (key) { return !allowedItemKeys.has(key); })
      ) {
        throw new TypeError("Featured language story fields are incomplete");
      }
      ...
      const thumbnailUrl = Object.prototype.hasOwnProperty.call(
        source,
        "thumbnail_url"
      ...
      if (
        ...
        !title ||
        !isSafeFeaturedCover(coverUrl) ||
        (thumbnailUrl && !isSafeFeaturedThumbnail(thumbnailUrl)) ||
        source.language !== language ||
        typeof source.episode_count !== "number" ||
        !Number.isInteger(source.episode_count) ||
        source.episode_count < 0
      ) {
        throw new TypeError("Featured language story is invalid");
      }
      seen.add(contentId);
      items.push(Object.freeze({
        content_id: contentId,
  ...
  function renderFeaturedStories(container, dramas, locale) {
    ...
    for (const [index, drama] of dramas.entries()) {
      ...
      const thumbnailUrl = isSafeFeaturedThumbnail(drama.thumbnail_url)
        ? drama.thumbnail_url
        : "";
      const coverUrl = isSafeFeaturedCover(drama.cover_url)
        ? drama.cover_url
        : "";
      if (thumbnailUrl || coverUrl) {
        let usedOriginalFallback = !thumbnailUrl;
        image.src = thumbnailUrl || coverUrl;
        image.addEventListener("error", function () {
          if (!usedOriginalFallback && coverUrl) {
  ...
  async function loadFeaturedStories(
    ...
    try {
      const result = await fetchPayload(
        buildFeaturedUrl(
          featuredLanguageForLocale(locale),
      ...
      const featured = normalizeFeaturedLanguagePayload(
        result.payload,
        featuredLanguageForLocale(locale)
      );
      ...
      renderFeaturedStories(container, buildFallbackFeatured(locale), locale);
  ...
  function applyPageCopy(locale) {
    ...
    const pageTitleLead = document.querySelector("#page-title-lead");
    const pageTitleAccent = document.querySelector("#page-title-accent");
    if (pageTitle && pageTitleLead && pageTitleAccent) {
      pageTitleLead.textContent = copyText(locale, "titleLead");
  ...
  function initPage() {
    ...
    const serverLocale = normalizeLanguageTag(
      document.documentElement.dataset.initialLocale
    );
    const hasServerLocale = Object.prototype.hasOwnProperty.call(
      COPY,
      serverLocale
    );
    // Accept-Language selected the localized HTML before first paint. Treat
    // that allowlisted marker as authoritative so deferred JS can never cause
    // a second visible language switch. Navigator is only a compatibility
    // fallback for legacy/direct HTML without a valid marker.
    const locale = hasServerLocale
      ? serverLocale
      : resolveUiLocale(browserLanguages);
    if (!hasServerLocale) {
  ...
  const api = Object.freeze({
    CODE_RESOLVER_PATH,
    FEATURED_PATH,
    FEATURED_LANGUAGE_SCHEMA_VERSION,
    TARGET_ORIGIN,
    TARGET_PATH,
    SEARCH_SOURCE,
    FEATURED_SOURCE,
    ...
    buildFeaturedUrl,
    buildFeaturedBundleUrl,
    validateTargetUrl,
    normalizeCodeResolvePayload,
    normalizeDramaPayload,
    isSafeFeaturedCover,
    isSafeFeaturedThumbnail,
    shanghaiYesterday,
    normalizeFeaturedPayload,
    normalizeFeaturedLanguagePayload,
