(function () {
  ...
  function renderRunTasks(tasks) {
    ...
    tasks.forEach(item => {
      ...
      accountCell.appendChild(ui.element("strong", { text: item.account_display_name || item.account_username || item.account_name || item.creator_nickname || `账号 ${item.account_id || item.source_account_id || "—"}` }));
      ...
      selectionCell.appendChild(ui.element("div", { text: `剧：${item.content_id || "—"}` }));
      selectionCell.appendChild(ui.element("div", { className: "secondary", text: `素材：${item.material_id || item.resource_id || "—"}` }));
      ...
      stageCell.appendChild(ui.element("div", { className: "secondary", text: item.publish_status || item.remote_status || (item.publish_id ? "已取得 publish_id" : "—") }));
      ...
      const publishId = item.publish_id || item.remote_publish_id;
      if (publishId) resultCell.appendChild(ui.element("div", { className: "mono", text: `publish_id ${publishId}` }));
      const postUrl = safeHttpsUrl(item.post_url || item.publish_url);
      if (postUrl) {
        const link = ui.element("a", { text: "打开 TikTok Post", href: postUrl, attributes: { target: "_blank", rel: "noopener noreferrer" } });
  ...
  function renderEvents(events) {
    ...
    events.forEach(item => {
      ...
      if (details) event.appendChild(ui.element("div", { className: "secondary", text: typeof details === "string" ? details : JSON.stringify(details) }));
  ...
  async function showRunDetail(id) {
    ...
    try {
      ...
      ui.setText(ui.byId("runSnapshot"), JSON.stringify(snapshot, null, 2), "{}");
      renderEvents(events);
    } catch (error) {
      if (request !== state.detailRequest) return;
