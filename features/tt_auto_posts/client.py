"""Secret-safe main-API client for the loopback TT auto-post sidecar."""
...
TT_AUTO_ADMIN_PREFIX = "/api/admin/tt-auto-publish"
DEFAULT_SERVICE_URL = "http://127.0.0.1:18831"
...
_GET_ROUTES = {
    f"{TT_AUTO_ADMIN_PREFIX}/accounts": set(),
    f"{TT_AUTO_ADMIN_PREFIX}/templates": {
        "status",
        "q",
        "limit",
        "offset",
    },
    f"{TT_AUTO_ADMIN_PREFIX}/runs": {
        "template_id",
        "trigger_type",
        "status",
        "from",
...
def _route_allowed(method: str, path: str) -> bool:
    if method == "GET":
        return path in _GET_ROUTES or bool(
            re.fullmatch(
                rf"{re.escape(TT_AUTO_ADMIN_PREFIX)}/(?:templates|runs)/[1-9][0-9]*",
                path,
            )
        )
    if method != "POST":
...
def parse_admin_query(path: str, raw_query: Any) -> Dict[str, str]:
    if path in _GET_ROUTES:
        ...
        raise TTAutoPostAdminClientError(
            "tt_auto_post_route_not_allowed",
            "TT 自动发布服务路由无效",
            500,
        )
    parsed = parse_qs(str(raw_query or ""), keep_blank_values=True)
    if set(parsed) - allowed:
        raise TTAutoPostAdminClientError("invalid_request", "查询参数无效", 400)
    result: Dict[str, str] = {}
...
def request_admin(
    ...
    *,
    ...
    service_url = str(
        source.get("TT_AUTO_POST_ADMIN_SERVICE_URL", DEFAULT_SERVICE_URL) or ""
    ).strip().rstrip("/")
    ...
    if (
        ...
        raise TTAutoPostAdminClientError(
            "tt_auto_post_service_not_configured",
            "TT 自动发布服务尚未完成内部配置",
            503,
        )
    if payload is not None and (
        ...
        raise TTAutoPostAdminClientError("invalid_request", "请求包含无效字段", 400)
    ...
    try:
        timeout = int(source.get("TT_AUTO_POST_ADMIN_TIMEOUT", "120") or "120")
    ...
    try:
        ...
        raise TTAutoPostAdminClientError(
            "tt_auto_post_service_unavailable",
            "TT 自动发布服务暂不可用",
            503,
        ) from None
    finally:
    ...
    if len(content) > MAX_RESPONSE_BYTES:
        raise TTAutoPostAdminClientError(
            "tt_auto_post_response_too_large",
            "TT 自动发布服务响应超过安全上限",
            502,
        )
    try:
    ...
    if not 200 <= int(response.status_code) < 300:
        ...
        raise TTAutoPostAdminClientError(
            code or "tt_auto_post_service_error",
...
def error_payload(exc: BaseException) -> tuple[int, Dict[str, Any]]:
    code = str(
        getattr(exc, "code", "tt_auto_post_service_unavailable")
        or "tt_auto_post_service_unavailable"
    )
    if not re.fullmatch(r"[a-z0-9_]{1,80}", code):
        code = "tt_auto_post_service_unavailable"
