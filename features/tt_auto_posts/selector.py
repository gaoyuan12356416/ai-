"""Deterministic two-stage TT automatic material selection.
...
legacy-history recheck, template cooldown and the global material uniqueness
...
MAX_SOURCE_DURATION_SECONDS = Decimal("3600")
...
class NoEligibleMaterial(SelectionError):
    def __init__(self, rejection_counts: Mapping[str, int]):
        ...
        super().__init__(
            "tt_auto_no_eligible_material",
            "no eligible TT material matched the template",
            409,
        )
...
class SelectionRules:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.platform, bool) or int(self.platform) < 0 or int(self.platform) > 255:
...
class LegacyMaterialReader(Protocol):
...
class MaterialValidator(Protocol):
    def validate(self, material_id: str) -> Mapping[str, Any]:
        ...


class ResolverMaterialValidator:
    """Adapt the hardened legacy resolver without importing its service class."""
    ...
    def validate(self, material_id: str) -> Mapping[str, Any]:
        try:
            ...
            code = str(getattr(exc, "code", "tt_auto_material_validation_failed"))
            if status >= 500:
                raise SelectionError(
                    "tt_auto_material_validator_unavailable",
...
def _validated_public_snapshot(
    ...
    *,
    ...
    source_media_url = value.get("source_media_url")
    if not isinstance(source_media_url, str) or not _https_url(source_media_url):
        raise SelectionError(
            "tt_auto_material_validator_invalid",
            "strict material validator returned an invalid source media URL",
            503,
        )
    result["source_media_url"] = source_media_url.strip()
    return result
...
def _material_snapshot(item: MaterialCandidate) -> Dict[str, Any]:
    result = {
        ...
        "media_url": item.source.media_url,
...
class TwoStageSelector:
    def __init__(
        ...
        legacy_reader: LegacyMaterialReader,
        ...
        *,
        ...
        if material_validator is None:
            raise ValueError("strict TT material validator is required")
        ...
        if (
            ...
            raise ValueError("TT automatic selection source scope is invalid")
    ...
    def _eligible_materials(
        ...
        legacy_seen = {str(value) for value in self.legacy_reader.seen_material_ids(ids)}
        auto_seen = {str(value) for value in self.store.reserved_material_ids(ids)}
        eligible: List[MaterialCandidate] = []
        for item in prepared:
            material_id = item.source.material_id
            if material_id in legacy_seen:
                self._reject(rejection_counts, "material_legacy_seen")
    ...
    def _validate_material(
        ...
        if (
            ...
            raise SelectionError(
                "tt_auto_material_validation_identity_mismatch",
    ...
    def select_and_reserve(self, request: SelectionRequest) -> ReservedSelection:
        existing = self.store.get_task_reservation(int(request.task_id))
        if existing is not None:
            existing_mapping = _object_mapping(existing)
            if existing_mapping is None:
                raise SelectionError(
                    "tt_auto_reservation_invalid",
        ...
        for drama in dramas:
            ...
            for material in materials:
                ...
                # Blacklist and legacy history can change after initial
                ...
                if material.source.material_id in {
                    str(value)
                    for value in self.legacy_reader.seen_material_ids(
                        [material.source.material_id]
                    )
                }:
                    self._reject(rejection_counts, "material_legacy_seen_final")
                    continue
                ...
                try:
                    ...
                    if conflict_code == "tt_auto_material_already_reserved":
                        self._reject(rejection_counts, "material_reservation_race")
                        continue
                    if conflict_code == "tt_auto_drama_in_cooldown":
