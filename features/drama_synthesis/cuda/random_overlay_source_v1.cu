...
extern "C" __global__ void compose(const uchar4* src,int w,int h,
    ...
    // Reuse this output frame's decoded source after every decorative layer.
    // Match the OpenCL cover fit and centered, unrotated zoom exactly.
    float overlay_u=(u-.5f)/SCENE_SOURCE_OVERLAY_SCALE+.5f;
    float overlay_v=(v-.5f)/SCENE_SOURCE_OVERLAY_SCALE+.5f;
    float4 source_overlay=cover(src,w,h,overlay_u,overlay_v);
