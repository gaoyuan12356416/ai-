    ...
    __read_only image2d_t tint)
{
    ...
    // Topmost synchronous copy of the untemplated source. First cover-fit,
    // then apply a centered zoom; the main plane's rotation never applies here.
    float2 overlay_uv = (uv - (float2)(0.5f, 0.5f)) / SCENE_SOURCE_OVERLAY_SCALE
                      + (float2)(0.5f, 0.5f);
    float4 source_overlay = sample_cover(source, overlay_uv);
