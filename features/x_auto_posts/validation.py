"""Strict public-input validation for X automatic publishing templates.

This module intentionally does not import the production X publishing service.
It mirrors the existing material-post body macro contract so the independent
control plane can validate and freeze a template without initializing any X
credentials, queues, or publishing state.
"""
...
MAX_X_AUTO_DURATION_SECONDS = 600
POST_TEMPLATE_MACRO_RE = re.compile(r"\{\{([a-z_]+)\}\}")
POST_TEMPLATE_ALLOWED_MACROS = frozenset({"drama_name", "desc", "url"})
LANGUAGE_RE = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)*$")
...
def _material_rule(raw: Any) -> Dict[str, Any]:
    ...
    minimum = _integer(
        value.get("duration_min_seconds"),
        "素材最小时长",
        0,
        MAX_X_AUTO_DURATION_SECONDS,
    )
    maximum = _integer(
        value.get("duration_max_seconds"),
        "素材最大时长",
        1,
        MAX_X_AUTO_DURATION_SECONDS,
    )
...
def _language(value: Any) -> str:
    language = _text(value, "语言", minimum=2, maximum=32).lower()
    if not LANGUAGE_RE.fullmatch(language):
        raise ValidationError("x_auto_language_invalid", "语言格式无效")
    return language


def _body_template(value: Any) -> str:
    template = _text(value, "X Post正文模板", minimum=1, maximum=2000)
    ...
    if "{{" in unmatched or "}}" in unmatched:
        raise ValidationError(
            ...
            "X Post正文模板包含不完整或格式无效的宏",
        )
    unknown = sorted(set(macros) - POST_TEMPLATE_ALLOWED_MACROS)
    if unknown:
        raise ValidationError(
            "x_auto_body_template_invalid",
            "X Post正文模板包含不支持的宏: %s" % "、".join(unknown),
        )
    missing = sorted({"drama_name", "desc"} - set(macros))
    if missing:
        raise ValidationError(
            "x_auto_body_template_invalid",
            "X Post正文模板缺少必需宏: %s" % "、".join(missing),
        )
    repeated = sorted(macro for macro in set(macros) if macros.count(macro) > 1)
    if repeated:
        raise ValidationError(
            "x_auto_body_template_invalid",
            "X Post正文模板宏不能重复: %s" % "、".join(repeated),
        )
    return template
