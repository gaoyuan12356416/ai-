  ...
  function renderScopeStep() {
    ...
    return `<section class="step-pane" aria-labelledby="scopeStepTitle">
      ...
      <div class="section-card"><div class="section-head"><div><h3>业务范围</h3><p>产品与优化师会同时进入服务端查询条件；广告账号不是配置项。</p></div></div>
        ...
        <div class="estimate-card"><div><h4>范围估算</h4><p>只返回聚合数量。选择产品、优化师、对象层级并显式填写指标窗口后可估算。</p><div class="field estimate-window"><label for="scopeMetricWindow">指标窗口 <span class="required" aria-hidden="true">*</span></label><div class="input-with-unit"><input id="scopeMetricWindow" type="number" min="1" inputmode="numeric" placeholder="输入最近天数" value="${h(editor.selection.metric_window_days || "")}" data-selection="metric_window_days"><span>天</span></div></div>${renderEstimateMetrics()}</div><button class="button" type="button" data-action="estimate-scope"${state.estimateLoading ? " disabled" : ""}>${state.estimateLoading ? "估算中…" : "估算当前范围"}</button></div>
        </div>
      </div>
    </section>`;
  }
