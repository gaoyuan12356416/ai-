...
const DETAIL_CACHE_NAME='tt-minis-native-growth-detail-v1', DETAIL_CACHE_TTL_MS=15*60*1000;
let detailCachePruned=false;
async function openDetailCache(){if(!('caches' in window))return null; try{let cache=await caches.open(DETAIL_CACHE_NAME); if(!detailCachePruned){detailCachePruned=true; let now=Date.now(), keys=await cache.keys(); await Promise.all(keys.map(async request=>{let response=await cache.match(request), cachedAt=Number(response&&response.headers.get('x-tt-cache-at')||0); if(!cachedAt||now-cachedAt>=DETAIL_CACHE_TTL_MS)await cache.delete(request);}));} return cache;}catch(e){return null;}}
async function fetchDailyJson(url){let cache=await openDetailCache(); if(cache){try{let cached=await cache.match(url), cachedAt=Number(cached&&cached.headers.get('x-tt-cache-at')||0); if(cached&&cachedAt&&Date.now()-cachedAt<DETAIL_CACHE_TTL_MS)return await cached.json(); if(cached)await cache.delete(url);}catch(e){}} let res=await fetch(url,{cache:'default',credentials:'same-origin'}); if(!res.ok)throw new Error('HTTP '+res.status); let text=await res.text(); if(cache){try{let headers=new Headers({'content-type':res.headers.get('content-type')||'application/json','x-tt-cache-at':String(Date.now())}); await cache.put(url,new Response(text,{status:200,headers}));}catch(e){}} return JSON.parse(text);}
