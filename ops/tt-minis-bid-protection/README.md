...
- Refresh: `--daily` refreshes the latest 30 completed Beijing calendar days. Existing `PAYMENT_COMPLETE` and `TARGET_MET` rows are not requested again; non-terminal and `INELIGIBLE` rows are refreshed. TikTok can later revise `INELIGIBLE` to `TARGET_MET`, so ineligibility is not permanently cached. Dates outside this window need a separate historical audit; `CONFIRMING` is pending, not a final zero-compensation decision.
...
WHERE protection_status IN ('UNDER_PROTECTION', 'CONFIRMING')
  AND record_date >= CURRENT_DATE - INTERVAL 30 DAY
