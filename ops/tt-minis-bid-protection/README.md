...
- Refresh: `--daily` refreshes the latest 30 completed Beijing calendar days. Existing terminal rows (`INELIGIBLE`, `PAYMENT_COMPLETE`, `TARGET_MET`) are not requested again; non-terminal rows are refreshed. Dates outside this window need a separate historical audit; `CONFIRMING` is pending, not a final zero-compensation decision.
